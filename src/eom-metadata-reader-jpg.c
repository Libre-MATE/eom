...
typedef enum { EJA_EXIF = 0, EJA_XMP, EJA_OTHER } EomJpegApp1Type;
...
#define IS_FINISHED(priv)                                 \
  (priv->state == EMR_READ && priv->exif_chunk != NULL && \
   priv->icc_chunk != NULL && priv->iptc_chunk != NULL && \
   priv->xmp_chunk != NULL)
...
static void eom_metadata_reader_jpg_init_emr_iface(gpointer g_iface,
                                                   gpointer iface_data);

G_DEFINE_TYPE_WITH_CODE(
    EomMetadataReaderJpg, eom_metadata_reader_jpg, G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE(EOM_TYPE_METADATA_READER,
                          eom_metadata_reader_jpg_init_emr_iface)
        G_ADD_PRIVATE(EomMetadataReaderJpg))

static void eom_metadata_reader_jpg_dispose(GObject *object) {
...
static void eom_metadata_reader_jpg_init(EomMetadataReaderJpg *emr) {
...
static void eom_metadata_reader_jpg_class_init(
    EomMetadataReaderJpgClass *klass) {
  GObjectClass *object_class = (GObjectClass *)klass;

  object_class->dispose = eom_metadata_reader_jpg_dispose;
}

static gboolean eom_metadata_reader_jpg_finished(EomMetadataReaderJpg *emr) {
  g_return_val_if_fail(EOM_IS_METADATA_READER_JPG(emr), TRUE);

  return (emr->priv->state == EMR_FINISHED);
}

static EomJpegApp1Type eom_metadata_identify_app1(gchar *buf, guint len) {
  if (len < 5) {
    return EJA_OTHER;
  }
...
static void eom_metadata_reader_get_next_block(
    EomMetadataReaderJpgPrivate *priv, guchar *chunk, int *i, const guchar *buf,
    int len, EomMetadataReaderState state) {
  if (*i + priv->size < len) {
...
static void eom_metadata_reader_jpg_consume(EomMetadataReaderJpg *emr,
    ...
    switch (priv->state) {
      ...
      case EMR_READ_MARKER:
        if ((buf[i] & 0xF0) == 0xE0 || buf[i] == 0xFE) {
          ...
          eom_debug_message(DEBUG_IMAGE_DATA, "APPx or COM Marker Found: %x",
                            priv->last_marker);
        } else {
      ...
      case EMR_READ_APP1:
        eom_debug_message(DEBUG_IMAGE_DATA, "Read APP1 data, Length: %i",
                          priv->size);
        ...
        if (chunk) {
          eom_metadata_reader_get_next_block(priv, chunk, &i, buf, len,
                                             next_state);
        }

        if (IS_FINISHED(priv)) priv->state = EMR_FINISHED;
        break;

      case EMR_READ_EXIF:
        eom_debug_message(DEBUG_IMAGE_DATA,
                          "Read continuation of EXIF data, length: %i",
                          priv->size);
        {
          eom_metadata_reader_get_next_block(priv, priv->exif_chunk, &i, buf,
                                             len, EMR_READ_EXIF);
        }
        if (IS_FINISHED(priv)) priv->state = EMR_FINISHED;
        break;
      ...
      case EMR_READ_XMP:
        eom_debug_message(DEBUG_IMAGE_DATA,
                          "Read continuation of XMP data, length: %i",
                          priv->size);
        {
          eom_metadata_reader_get_next_block(priv, priv->xmp_chunk, &i, buf,
                                             len, EMR_READ_XMP);
        }
        if (IS_FINISHED(priv)) priv->state = EMR_FINISHED;
        break;
      ...
      case EMR_READ_ICC:
        ...
        eom_metadata_reader_get_next_block(priv, priv->icc_chunk, &i, buf, len,
                                           EMR_READ_ICC);
        ...
        if (priv->state == EMR_READ && priv->icc_chunk) {
          ...
          valid &= strncmp(icc_chunk, "ICC_PROFILE\0", 12) == 0;
      ...
      case EMR_READ_IPTC:
        ...
        eom_metadata_reader_get_next_block(priv, priv->iptc_chunk, &i, buf, len,
                                           EMR_READ_IPTC);
 ...
 */
static void eom_metadata_reader_jpg_get_exif_chunk(EomMetadataReaderJpg *emr,
...
#ifdef HAVE_EXIF
static gpointer eom_metadata_reader_jpg_get_exif_data(
    EomMetadataReaderJpg *emr) {
  EomMetadataReaderJpgPrivate *priv;
  ExifData *data = NULL;
...
static gpointer eom_metadata_reader_jpg_get_xmp_data(
    EomMetadataReaderJpg *emr) {
  EomMetadataReaderJpgPrivate *priv;
  XmpPtr xmp = NULL;
...
static gpointer eom_metadata_reader_jpg_get_icc_profile(
    EomMetadataReaderJpg *emr) {
  EomMetadataReaderJpgPrivate *priv;
...
static void eom_metadata_reader_jpg_init_emr_iface(gpointer g_iface,
  ...
  iface->finished =
      (gboolean(*)(EomMetadataReader * self)) eom_metadata_reader_jpg_finished;
  iface->get_raw_exif =
      (void (*)(EomMetadataReader * self, guchar * *data, guint * len))
          eom_metadata_reader_jpg_get_exif_chunk;
#ifdef HAVE_EXIF
  iface->get_exif_data = (gpointer(*)(EomMetadataReader * self))
      eom_metadata_reader_jpg_get_exif_data;
#endif
#if defined(HAVE_LCMS) && defined(GDK_WINDOWING_X11)
  iface->get_icc_profile = (gpointer(*)(EomMetadataReader * self))
