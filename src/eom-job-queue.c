...
static gboolean remove_job_from_queue(GQueue *queue, EomJob *job) {
  GList *list;
...
static void add_job_to_queue_locked(GQueue *queue, EomJob *job) {
  g_object_ref(job);
  g_queue_push_tail(queue, job);
  g_cond_broadcast(&render_cond);
}

static gboolean notify_finished(GObject *job) {
  eom_job_finished(EOM_JOB(job));

  return FALSE;
}

static void handle_job(EomJob *job) {
  g_object_ref(G_OBJECT(job));

  // Do the EOM_JOB cast for safety
  eom_job_run(EOM_JOB(job));

  g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, (GSourceFunc)notify_finished, job,
                  g_object_unref);
}

static gboolean no_jobs_available_unlocked(void) {
  return g_queue_is_empty(load_queue) && g_queue_is_empty(transform_queue) &&
         g_queue_is_empty(thumbnail_queue) && g_queue_is_empty(model_queue) &&
         g_queue_is_empty(save_queue) && g_queue_is_empty(copy_queue);
}

static EomJob *search_for_jobs_unlocked(void) {
  EomJob *job;
...
static gpointer eom_render_thread(gpointer data) {
  while (TRUE) {
    EomJob *job;
...
static GQueue *find_queue(EomJob *job) {
