...
enum { SIGNAL_PAGE_ADDED, SIGNAL_PAGE_REMOVED, SIGNAL_LAST };
...
static void eom_sidebar_destroy(GtkWidget *object) {
...
static void eom_sidebar_select_page(EomSidebar *eom_sidebar,
                                    GtkTreeIter *iter) {
  gchar *title;
  gint index;

  gtk_tree_model_get(eom_sidebar->priv->page_model, iter, PAGE_COLUMN_TITLE,
...
void eom_sidebar_set_page(EomSidebar *eom_sidebar, GtkWidget *main_widget) {
  GtkTreeIter iter;
  gboolean valid;
...
static GtkWidget *eom_sidebar_get_current_page(EomSidebar *sidebar) {
...
static void eom_sidebar_set_property(GObject *object, guint prop_id,
...
static void eom_sidebar_get_property(GObject *object, guint prop_id,
...
static void eom_sidebar_class_init(EomSidebarClass *eom_sidebar_class) {
  ...
  signals[SIGNAL_PAGE_ADDED] = g_signal_new(
      "page-added", EOM_TYPE_SIDEBAR, G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(EomSidebarClass, page_added), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GTK_TYPE_WIDGET);

  signals[SIGNAL_PAGE_REMOVED] = g_signal_new(
      "page-removed", EOM_TYPE_SIDEBAR, G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(EomSidebarClass, page_removed), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GTK_TYPE_WIDGET);
}

static gboolean eom_sidebar_select_button_press_cb(GtkWidget *widget,
  ...
  if (event->button == 1) {
    ...
    gtk_widget_set_size_request(eom_sidebar->priv->menu,
                                MAX(allocation.width, requisition.width), -1);

    gtk_widget_grab_focus(widget);

    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), TRUE);

    gtk_menu_popup_at_widget(GTK_MENU(eom_sidebar->priv->menu), widget,
...
static gboolean eom_sidebar_select_button_key_press_cb(GtkWidget *widget,
                                                       GdkEventKey *event,
                                                       gpointer user_data) {
  EomSidebar *eom_sidebar = EOM_SIDEBAR(user_data);
...
static void eom_sidebar_close_clicked_cb(GtkWidget *widget,
                                         gpointer user_data) {
  EomSidebar *eom_sidebar = EOM_SIDEBAR(user_data);
...
static void eom_sidebar_menu_deactivate_cb(GtkWidget *widget,
                                           gpointer user_data) {
...
static void eom_sidebar_menu_detach_cb(GtkWidget *widget, GtkMenu *menu) {
  EomSidebar *eom_sidebar = EOM_SIDEBAR(widget);

  eom_sidebar->priv->menu = NULL;
}

static void eom_sidebar_menu_item_activate_cb(GtkWidget *widget,
                                              gpointer user_data) {
  EomSidebar *eom_sidebar = EOM_SIDEBAR(user_data);
  GtkTreeIter iter;
...
static void eom_sidebar_update_arrow_visibility(EomSidebar *sidebar) {
  EomSidebarPrivate *priv = sidebar->priv;
  const gint n_pages = eom_sidebar_get_n_pages(sidebar);

  gtk_widget_set_visible(GTK_WIDGET(priv->arrow), n_pages > 1);
}

static void eom_sidebar_init(EomSidebar *eom_sidebar) {
  GtkWidget *hbox;
  ...
  /* data model */
  eom_sidebar->priv->page_model = (GtkTreeModel *)gtk_list_store_new(
  ...
  g_signal_connect(eom_sidebar->priv->select_button, "button_press_event",
                   G_CALLBACK(eom_sidebar_select_button_press_cb), eom_sidebar);
  ...
  gtk_box_pack_start(GTK_BOX(select_hbox), eom_sidebar->priv->label, FALSE,
                     FALSE, 0);
  ...
  gtk_container_add(GTK_CONTAINER(eom_sidebar->priv->select_button),
                    select_hbox);
  ...
  gtk_notebook_set_show_border(GTK_NOTEBOOK(eom_sidebar->priv->notebook),
                               FALSE);
  gtk_notebook_set_show_tabs(GTK_NOTEBOOK(eom_sidebar->priv->notebook), FALSE);

  gtk_box_pack_start(GTK_BOX(eom_sidebar), eom_sidebar->priv->notebook, TRUE,
                     TRUE, 0);
...
GtkWidget *eom_sidebar_new(void) {
  ...
  eom_sidebar = g_object_new(EOM_TYPE_SIDEBAR, "orientation",
                             GTK_ORIENTATION_VERTICAL, NULL);

  return eom_sidebar;
}

void eom_sidebar_add_page(EomSidebar *eom_sidebar, const gchar *title,
  ...
  g_signal_connect(menu_item, "activate",
                   G_CALLBACK(eom_sidebar_menu_item_activate_cb), eom_sidebar);

  gtk_widget_show(menu_item);

  gtk_menu_shell_append(GTK_MENU_SHELL(eom_sidebar->priv->menu), menu_item);

  /* Insert and move to end */
  gtk_list_store_insert_with_values(
      GTK_LIST_STORE(eom_sidebar->priv->page_model), &iter, 0,
      PAGE_COLUMN_TITLE, title, PAGE_COLUMN_MENU_ITEM, menu_item,
      PAGE_COLUMN_MAIN_WIDGET, main_widget, PAGE_COLUMN_NOTEBOOK_INDEX, index,
      -1);
...
void eom_sidebar_remove_page(EomSidebar *eom_sidebar, GtkWidget *main_widget) {
  GtkTreeIter iter;
  ...
  if (valid) {
    gtk_notebook_remove_page(GTK_NOTEBOOK(eom_sidebar->priv->notebook), index);

    gtk_container_remove(GTK_CONTAINER(eom_sidebar->priv->menu), menu_item);

    gtk_list_store_remove(GTK_LIST_STORE(eom_sidebar->priv->page_model), &iter);
