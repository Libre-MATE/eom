...
static void eom_transform_init(EomTransform *trans) {
  trans->priv = eom_transform_get_instance_private(trans);
}

static void eom_transform_class_init(EomTransformClass *klass) {}
 ...
 * @job: a #EomJob
 *
 * Applies the transformation in @trans to @pixbuf, setting its progress in
 *@job.
 *
 * Returns: (transfer full): A new #GdkPixbuf with the transformation applied.
 **/
GdkPixbuf *eom_transform_apply(EomTransform *trans, GdkPixbuf *pixbuf,
                               EomJob *job) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    cairo_matrix_transform_point(&trans->priv->affine, &dest.x, &dest.y);
  ...
  dest_pixbuf = gdk_pixbuf_new(
      ...
      gdk_pixbuf_get_bits_per_sample(pixbuf), dest_width, dest_height);
  ...
  /* invert the matrix so that we can compute the source pixel
     ...
     available.
  */
  r_det = 1.0 / (trans->priv->affine.xx * trans->priv->affine.yy -
                 trans->priv->affine.yx * trans->priv->affine.xy);
  ...
  inverted[4] = -trans->priv->affine.x0 * inverted[0] -
                trans->priv->affine.y0 * inverted[2];
  inverted[5] = -trans->priv->affine.x0 * inverted[1] -
                trans->priv->affine.y0 * inverted[3];
...
static void _eom_cairo_matrix_copy(const cairo_matrix_t *src,
                                   cairo_matrix_t *dest) {
...
static gboolean _eom_cairo_matrix_equal(const cairo_matrix_t *a,
...
EomTransform *eom_transform_reverse(EomTransform *trans) {
  ...
  g_return_val_if_fail(
      cairo_matrix_invert(&reverse->priv->affine) == CAIRO_STATUS_SUCCESS,
      reverse);
...
EomTransform *eom_transform_compose(EomTransform *trans,
  ...
  cairo_matrix_multiply(&composition->priv->affine, &trans->priv->affine,
                        &compose->priv->affine);
...
EomTransform *eom_transform_flip_new(EomTransformType type) {
  ...
  _eom_cairo_matrix_flip(&trans->priv->affine, &trans->priv->affine, horiz,
                         vert);

  return trans;
}

EomTransform *eom_transform_new(EomTransformType type) {
