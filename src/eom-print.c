...
static void _eom_cairo_matrix_flip(cairo_matrix_t *dst,
                                   const cairo_matrix_t *src, gboolean horiz,
                                   gboolean vert) {
...
static gboolean _cairo_ctx_supports_jpg_metadata(cairo_t *cr) {
...
static void eom_print_draw_page(GtkPrintOperation *operation,
  ...
  cairo_rectangle(cr, 0, 0, MIN(width * scale_factor, p_width),
                  MIN(height * scale_factor, p_height));
  cairo_clip(cr);
...
#endif
    /* JPEGs can be attached to the cairo surface which simply embeds the JPEG
     * file into the destination PDF skipping (PNG-)recompression. This should
     * reduce PDF sizes enormously. */
    if (eom_image_is_jpeg(data->image) &&
        _cairo_ctx_supports_jpg_metadata(cr)) {
      GFile *file;
      char *img_data;
      gsize data_len;
      ...
      if (g_file_load_contents(file, NULL, &img_data, &data_len, NULL, NULL)) {
        ...
        /* Care must be taken with height and width values. They are not the
         * original values but were affected by the transformation. As the
         * surface needs to be generated using the original dimensions they
        ...
        if (tf) {
          ...
          switch (eom_transform_get_transform_type(tf)) {
            case EOM_TRANSFORM_ROT_90:
              surface =
                  cairo_image_surface_create(CAIRO_FORMAT_RGB24, height, width);
        ...
        cairo_surface_set_mime_data(surface, CAIRO_MIME_TYPE_JPEG,
                                    (unsigned char *)img_data, data_len, g_free,
                                    img_data);
...
static GObject *eom_print_create_custom_widget(GtkPrintOperation *operation,
                                               gpointer user_data) {
  GtkPageSetup *page_setup;
  EomPrintData *data;
  ...
  if (page_setup == NULL) page_setup = gtk_page_setup_new();

  return G_OBJECT(eom_print_image_setup_new(data->image, page_setup));
}

static void eom_print_custom_widget_apply(GtkPrintOperation *operation,
                                          GtkWidget *widget,
                                          gpointer user_data) {
  EomPrintData *data;
  ...
  eom_print_image_setup_get_options(EOM_PRINT_IMAGE_SETUP(widget), &left_margin,
                                    &top_margin, &scale_factor, &unit);
...
static void eom_print_end_print(GtkPrintOperation *operation,
...
GtkPrintOperation *eom_print_operation_new(EomImage *image,
  ...
  gtk_print_operation_set_job_name(print, eom_image_get_caption(image));
  gtk_print_operation_set_embed_page_setup(print, TRUE);

  g_signal_connect(print, "draw_page", G_CALLBACK(eom_print_draw_page), data);
  g_signal_connect(print, "create-custom-widget",
                   G_CALLBACK(eom_print_create_custom_widget), data);
  g_signal_connect(print, "custom-widget-apply",
                   G_CALLBACK(eom_print_custom_widget_apply), data);
  g_signal_connect(print, "end-print", G_CALLBACK(eom_print_end_print), data);
  g_signal_connect(print, "update-custom-widget",
                   G_CALLBACK(eom_print_image_setup_update), data);
...
static GKeyFile *eom_print_get_key_file(void) {
  GKeyFile *key_file;
  GError *error = NULL;
  gchar *filename;
  GFile *file;
...
static void eom_print_save_key_file(GKeyFile *key_file) {
  gchar *filename;
  gchar *data;
  GError *error = NULL;
  const gchar *dot_dir = eom_util_dot_dir();
