...
/* Signal IDs */
enum { SIGNAL_IMAGE_MOVED, SIGNAL_LAST };
...
G_DEFINE_TYPE_WITH_PRIVATE(EomPrintPreview, eom_print_preview,
...
static void eom_print_preview_get_property(GObject *object, guint prop_id,
...
static void eom_print_preview_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
  EomPrintPreviewPrivate *priv = EOM_PRINT_PREVIEW(object)->priv;
  ...
  if (paper_size_changed) {
    g_object_set(object, "ratio", priv->p_width / priv->p_height, NULL);
  }
...
static void eom_print_preview_class_init(EomPrintPreviewClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("image", "Image to show in the preview", "",
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_IMAGE_X_ALIGN,
      g_param_spec_float("image-x-align", "Horizontal alignment for the image",
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_IMAGE_Y_ALIGN,
      g_param_spec_float("image-y-align", "Vertical alignment for the image",
                         "", 0, 1, 0.5, G_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_IMAGE_SCALE,
      g_param_spec_float("image-scale", "The scale for the image", "", 0, 1, 1,
                         G_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_PAPER_WIDTH,
      g_param_spec_float("paper-width", "Real paper width in inches", "", 0,
                         100, 8.5, G_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_PAPER_HEIGHT,
      g_param_spec_float("paper-height", "Real paper height in inches", "", 0,
                         200, 11, G_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_PAGE_LEFT_MARGIN,
      g_param_spec_float("page-left-margin",
                         "Left margin of the page in inches", "", 0, 100, 0.25,
                         G_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_PAGE_RIGHT_MARGIN,
      g_param_spec_float("page-right-margin",
                         "Right margin of the page in inches", "", 0, 200, 0.25,
                         G_PARAM_READWRITE));
  /**
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_PAGE_TOP_MARGIN,
      g_param_spec_float("page-top-margin", "Top margin of the page in inches",
                         "", 0, 100, 0.25, G_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_PAGE_BOTTOM_MARGIN,
      g_param_spec_float("page-bottom-margin",
                         "Bottom margin of the page in inches", "", 0, 200,
                         0.56, G_PARAM_READWRITE));
   ...
   */
  preview_signals[SIGNAL_IMAGE_MOVED] = g_signal_new(
      "image_moved", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_FIRST, 0,
...
static void eom_print_preview_init(EomPrintPreview *preview) {
  EomPrintPreviewPrivate *priv;
  gfloat ratio;
...
static gboolean button_press_event_cb(GtkWidget *widget, GdkEventButton *bev,
                                      gpointer user_data);
static gboolean button_release_event_cb(GtkWidget *widget, GdkEventButton *bev,
                                        gpointer user_data);
static gboolean motion_notify_event_cb(GtkWidget *widget, GdkEventMotion *mev,
                                       gpointer user_data);
static gboolean key_press_event_cb(GtkWidget *widget, GdkEventKey *event,
                                   gpointer user_data);

static gboolean draw_cb(GtkDrawingArea *drawing_area, cairo_t *cr,
                        gpointer user_data);
...
GtkWidget *eom_print_preview_new(void) {
  ...
  g_object_set(G_OBJECT(area), "can-focus", TRUE, NULL);

  /* 	update_relative_sizes (preview); */

  g_signal_connect(area, "draw", G_CALLBACK(draw_cb), preview);
  ...
  g_signal_connect(area, "key-press-event", G_CALLBACK(key_press_event_cb),
                   preview);

  g_signal_connect(area, "size-allocate", G_CALLBACK(size_allocate_cb),
                   preview);
 ...
 **/
static void get_current_image_coordinates(EomPrintPreview *preview, gint *x0,
                                          gint *y0) {
  EomPrintPreviewPrivate *priv;
  ...
  *x0 = (gint)((1 - priv->image_x_align) * priv->l_rmargin +
               priv->image_x_align *
                   (allocation.width - priv->r_rmargin - priv->r_width));
  *y0 = (gint)((1 - priv->image_y_align) * priv->t_rmargin +
               priv->image_y_align *
                   (allocation.height - priv->b_rmargin - priv->r_height));
}
 ...
 * %FALSE otherwise.
 **/
static gboolean press_inside_image_area(EomPrintPreview *preview, guint x,
                                        guint y) {
  EomPrintPreviewPrivate *priv;
  gint x0, y0;
...
static void create_image_scaled(EomPrintPreview *preview) {
  ...
  if (!priv->image_scaled) {
    ...
    if ((i_width > allocation.width) || (i_height > allocation.height)) {
      ...
      priv->image_scaled = gdk_pixbuf_scale_simple(
          priv->image, i_width * scale, i_height * scale, GDK_INTERP_TILES);
    } else {
...
static GdkPixbuf *create_preview_buffer(EomPrintPreview *preview) {
  GdkPixbuf *pixbuf;
  ...
  width *= preview->priv->i_scale * preview->priv->p_scale * widget_scale;
  height *= preview->priv->i_scale * preview->priv->p_scale * widget_scale;

  if (width < 1 || height < 1) return NULL;

  /* to use GDK_INTERP_TILES for small pixbufs is expensive and unnecessary */
  if (width < 25 || height < 25) type = GDK_INTERP_NEAREST;

  if (preview->priv->image_scaled) {
    pixbuf = gdk_pixbuf_scale_simple(preview->priv->image_scaled, width, height,
                                     type);
  } else {
    pixbuf = gdk_pixbuf_scale_simple(preview->priv->image, width, height, type);
  }
...
static void create_surface(EomPrintPreview *preview) {
  ...
  if (pixbuf) {
    priv->surface = gdk_cairo_surface_create_from_pixbuf(
        pixbuf, 0, gtk_widget_get_window(GTK_WIDGET(preview)));
...
static gboolean button_press_event_cb(GtkWidget *widget, GdkEventButton *event,
  ...
  switch (event->button) {
    case 1:
      preview->priv->grabbed =
          press_inside_image_area(preview, event->x, event->y);
      break;
  }
...
static gboolean motion_notify_event_cb(GtkWidget *widget, GdkEventMotion *event,
  ...
  if (priv->grabbed) {
    ...
    priv->image_x_align +=
        (dx + priv->r_dx) /
        (allocation.width - priv->r_width - priv->l_rmargin - priv->r_rmargin);
    ...
    priv->image_y_align +=
        (dy + priv->r_dy) / (allocation.height - priv->r_height -
    ...
    g_object_set(EOM_PRINT_PREVIEW(user_data), "image-x-align",
                 priv->image_x_align, "image-y-align", priv->image_y_align,
                 NULL);
    ...
    if (press_inside_image_area(EOM_PRINT_PREVIEW(user_data), event->x,
                                event->y)) {
      GdkCursor *cursor;
      cursor =
...
static void eom_print_preview_draw(EomPrintPreview *preview, cairo_t *cr) {
  ...
  cairo_rectangle(cr, priv->l_rmargin, priv->t_rmargin,
  ...
  if (priv->surface) {
    ...
    gdouble scale = priv->i_scale * priv->p_scale *
                    ...
                    gdk_pixbuf_get_width(priv->image_scaled);
    x0 /= scale;
    y0 /= scale;
    ...
    cairo_scale(cr, priv->i_scale * priv->p_scale,
                priv->i_scale * priv->p_scale);
  ...
  if (has_focus) {
    ...
    gtk_render_focus(ctx, cr, 0, 0, allocation.width, allocation.height);
  }
}
...
void eom_print_preview_set_page_margins(EomPrintPreview *preview,
                                        gfloat l_margin, gfloat r_margin,
                                        gfloat t_margin, gfloat b_margin) {
  g_return_if_fail(EOM_IS_PRINT_PREVIEW(preview));

  g_object_set(G_OBJECT(preview), "page-left-margin", l_margin,
               "page-right-margin", r_margin, "page-top-margin", t_margin,
               "page-bottom-margin", r_margin, NULL);
}
...
void eom_print_preview_set_from_page_setup(EomPrintPreview *preview,
  ...
  g_object_set(
      G_OBJECT(preview), "page-left-margin",
      gtk_page_setup_get_left_margin(setup, GTK_UNIT_INCH), "page-right-margin",
      gtk_page_setup_get_right_margin(setup, GTK_UNIT_INCH), "page-top-margin",
      gtk_page_setup_get_top_margin(setup, GTK_UNIT_INCH), "page-bottom-margin",
      gtk_page_setup_get_bottom_margin(setup, GTK_UNIT_INCH), "paper-width",
      gtk_page_setup_get_paper_width(setup, GTK_UNIT_INCH), "paper-height",
      gtk_page_setup_get_paper_height(setup, GTK_UNIT_INCH), NULL);
}
 ...
 **/
void eom_print_preview_get_image_position(EomPrintPreview *preview, gdouble *x,
                                          gdouble *y) {
  EomPrintPreviewPrivate *priv;
 ...
 **/
void eom_print_preview_set_image_position(EomPrintPreview *preview, gdouble x,
                                          gdouble y) {
  EomPrintPreviewPrivate *priv;
  ...
  if (x != -1) {
    width = gdk_pixbuf_get_width(priv->image) * priv->i_scale / 72.;
    x_align = CLAMP(
        x / (priv->p_width - priv->l_margin - priv->r_margin - width), 0, 1);
  ...
  if (y != -1) {
    height = gdk_pixbuf_get_height(priv->image) * priv->i_scale / 72.;
    y_align = CLAMP(
        y / (priv->p_height - priv->t_margin - priv->b_margin - height), 0, 1);
 ...
 **/
void eom_print_preview_set_scale(EomPrintPreview *preview, gfloat scale) {
  g_return_if_fail(EOM_IS_PRINT_PREVIEW(preview));

  g_object_set(preview, "image-scale", scale, NULL);
}
