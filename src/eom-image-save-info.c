...
static void eom_image_save_info_dispose(GObject *object) {
...
static void eom_image_save_info_init(EomImageSaveInfo *obj) {}

static void eom_image_save_info_class_init(EomImageSaveInfoClass *klass) {
 ...
 * filesystem calls.
 */
static gboolean is_local_file(GFile *file) {
  char *scheme;
  gboolean ret;
...
static char *get_save_file_type_by_file(GFile *file) {
...
EomImageSaveInfo *eom_image_save_info_new_from_uri(const char *txt_uri,
                                                   GdkPixbufFormat *format) {
  GFile *file;
