...
static gboolean thumbview_on_query_tooltip_cb(GtkWidget *widget, gint x, gint y,
                                              gboolean keyboard_mode,
                                              GtkTooltip *tooltip,
                                              gpointer user_data);
static void thumbview_on_parent_set_cb(GtkWidget *widget, GtkWidget *old_parent,
                                       gpointer user_data);

static void thumbview_on_drag_data_get_cb(GtkWidget *widget,
...
static void eom_thumb_view_constructed(GObject *object) {
  ...
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(thumbview),
                             thumbview->priv->pixbuf_cell, FALSE);

  g_object_set(thumbview->priv->pixbuf_cell, "height", 100, "width", 115,
               "yalign", 0.5, "xalign", 0.5, NULL);

  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(thumbview),
                                 thumbview->priv->pixbuf_cell, "pixbuf",
                                 EOM_LIST_STORE_THUMBNAIL, NULL);
  ...
  g_signal_connect(thumbview, "query-tooltip",
                   G_CALLBACK(thumbview_on_query_tooltip_cb), NULL);
  ...
  gtk_icon_view_enable_model_drag_source(
      GTK_ICON_VIEW(thumbview), 0, NULL, 0,
...
static void eom_thumb_view_dispose(GObject *object) {
...
static void eom_thumb_view_get_property(GObject *object, guint prop_id,
...
static void eom_thumb_view_set_property(GObject *object, guint prop_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
  EomThumbView *view = EOM_THUMB_VIEW(object);
...
static void eom_thumb_view_class_init(EomThumbViewClass *class) {
...
static void eom_thumb_view_clear_range(EomThumbView *thumbview,
                                       const gint start_thumb,
                                       const gint end_thumb) {
  GtkTreePath *path;
  GtkTreeIter iter;
  EomListStore *store =
      EOM_LIST_STORE(gtk_icon_view_get_model(GTK_ICON_VIEW(thumbview)));
...
static void eom_thumb_view_add_range(EomThumbView *thumbview,
                                     const gint start_thumb,
                                     const gint end_thumb) {
  GtkTreePath *path;
  GtkTreeIter iter;
...
static void eom_thumb_view_update_visible_range(EomThumbView *thumbview,
                                                const gint start_thumb,
                                                const gint end_thumb) {
  ...
  if (start_thumb == old_start_thumb && end_thumb == old_end_thumb) {
    return;
  }

  if (old_start_thumb < start_thumb)
    eom_thumb_view_clear_range(thumbview, old_start_thumb,
                               MIN(start_thumb - 1, old_end_thumb));

  if (old_end_thumb > end_thumb)
    eom_thumb_view_clear_range(thumbview, MAX(end_thumb + 1, old_start_thumb),
                               old_end_thumb);
...
static gboolean visible_range_changed_cb(EomThumbView *thumbview) {
  GtkTreePath *path1, *path2;

  thumbview->priv->visible_range_changed_id = 0;

  if (!gtk_icon_view_get_visible_range(GTK_ICON_VIEW(thumbview), &path1,
                                       &path2)) {
    return FALSE;
  }
  ...
  if (path2 == NULL) {
    gint n_items = gtk_tree_model_iter_n_children(
        gtk_icon_view_get_model(GTK_ICON_VIEW(thumbview)), NULL);
    path2 = gtk_tree_path_new_from_indices(n_items - 1, -1);
  }

  eom_thumb_view_update_visible_range(thumbview,
...
static void eom_thumb_view_visible_range_changed(EomThumbView *thumbview) {
  if (thumbview->priv->visible_range_changed_id == 0) {
    g_idle_add((GSourceFunc)visible_range_changed_cb, thumbview);
  }
}

static void thumbview_on_visible_range_changed_cb(EomThumbView *thumbview,
                                                  gpointer user_data) {
  eom_thumb_view_visible_range_changed(thumbview);
}

static void thumbview_on_adjustment_changed_cb(EomThumbView *thumbview,
                                               gpointer user_data) {
  eom_thumb_view_visible_range_changed(thumbview);
}
...
static gboolean thumbview_on_button_press_event_cb(GtkWidget *thumbview,
                                                   GdkEventButton *event,
                                                   gpointer user_data) {
  GtkTreePath *path;
...
static gchar *thumbview_get_tooltip_string(EomImage *image) {
  gchar *bytes;
  char *type_str;
  gint width, height;
  GFile *file;
  ...
  if (width > -1 && height > -1) {
    tooltip_string = g_markup_printf_escaped(
        "<b><big>%s</big></b>\n"
        "%i x %i %s\n"
        "%s\n"
        "%s",
        eom_image_get_caption(image), width, height,
        ngettext("pixel", "pixels", height), bytes, type_str);
  } else {
    tooltip_string = g_markup_printf_escaped(
        "<b><big>%s</big></b>\n"
        "%s\n"
        "%s",
        eom_image_get_caption(image), bytes, type_str);
  }
  ...
  if (exif_data) {
    ...
    date = eom_exif_util_format_date(eom_exif_data_get_value(
        exif_data, EXIF_TAG_DATE_TIME_ORIGINAL, time_buffer, 32));
...
static void on_data_loaded_cb(EomJob *job, gpointer data) {
  if (!job->error) {
...
static gboolean thumbview_on_query_tooltip_cb(GtkWidget *widget, gint x, gint y,
  ...
  if (!gtk_icon_view_get_tooltip_context(GTK_ICON_VIEW(widget), &x, &y,
                                         keyboard_mode, NULL, &path, NULL)) {
    return FALSE;
  }

  image = eom_thumb_view_get_image_from_path(EOM_THUMB_VIEW(widget), path);
  gtk_tree_path_free(path);
  ...
  if (data) {
    ...
    g_signal_connect(job, "finished", G_CALLBACK(on_data_loaded_cb), widget);
...
static void eom_thumb_view_update_columns(EomThumbView *view) {
  ...
  if (priv->orientation == GTK_ORIENTATION_HORIZONTAL)
    gtk_icon_view_set_columns(GTK_ICON_VIEW(view), priv->n_images);
}

static void eom_thumb_view_row_inserted_cb(GtkTreeModel *tree_model,
...
static void eom_thumb_view_row_deleted_cb(GtkTreeModel *tree_model,
                                          GtkTreePath *path,
                                          EomThumbView *view) {
  EomThumbViewPrivate *priv = view->priv;
 ...
 *
 **/
void eom_thumb_view_set_model(EomThumbView *thumbview, EomListStore *store) {
  gint index;
  EomThumbViewPrivate *priv;
  ...
  if (existing != NULL) {
    if (priv->image_add_id != 0) {
      g_signal_handler_disconnect(existing, priv->image_add_id);
    }
    if (priv->image_removed_id != 0) {
      g_signal_handler_disconnect(existing, priv->image_removed_id);
    }
  }

  priv->image_add_id =
      g_signal_connect(store, "row-inserted",
                       G_CALLBACK(eom_thumb_view_row_inserted_cb), thumbview);
  priv->image_removed_id =
      g_signal_connect(store, "row-deleted",
                       G_CALLBACK(eom_thumb_view_row_deleted_cb), thumbview);
 ...
 *
 **/
void eom_thumb_view_set_item_height(EomThumbView *thumbview, gint height) {
  g_return_if_fail(EOM_IS_THUMB_VIEW(thumbview));

  g_object_set(thumbview->priv->pixbuf_cell, "height", height, NULL);
}

static void eom_thumb_view_get_n_selected_helper(GtkIconView *thumbview,
                                                 GtkTreePath *path,
                                                 gpointer data) {
...
guint eom_thumb_view_get_n_selected(EomThumbView *thumbview) {
  ...
  gtk_icon_view_selected_foreach(
      GTK_ICON_VIEW(thumbview), eom_thumb_view_get_n_selected_helper, (&count));
  return count;
}
 ...
 *
 * Gets the #EomImage stored in @thumbview's #EomListStore at the position
 *indicated by @path.
 *
...
static EomImage *eom_thumb_view_get_image_from_path(EomThumbView *thumbview,
  ...
  gtk_tree_model_get(model, &iter, EOM_LIST_STORE_EOM_IMAGE, &image, -1);
 ...
 *
 * Returns: (element-type EomImage) (transfer full): A newly allocated list of
 *#EomImage's.
 **/
GList *eom_thumb_view_get_selected_images(EomThumbView *thumbview) {
  ...
  for (item = l; item != NULL; item = item->next) {
    ...
    list = g_list_prepend(list,
                          eom_thumb_view_get_image_from_path(thumbview, path));
    gtk_tree_path_free(path);
  }
 ...
 *
 **/
void eom_thumb_view_set_current_image(EomThumbView *thumbview, EomImage *image,
  ...
  if (eom_thumb_view_get_n_selected(thumbview) == 0) {
    switch (change) {
      ...
      case EOM_THUMB_VIEW_SELECT_RANDOM:
        path =
            gtk_tree_path_new_from_indices(g_random_int_range(0, n_items), -1);
        break;
    }
  } else {
