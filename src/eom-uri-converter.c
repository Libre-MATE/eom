...
static void eom_uri_converter_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);

static void eom_uri_converter_get_property(GObject *object, guint property_id,
...
static void eom_uri_converter_dispose(GObject *object) {
...
static void eom_uri_converter_init(EomURIConverter *conv) {
  EomURIConverterPrivate *priv;
...
static void eom_uri_converter_class_init(EomURIConverterClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_CONVERT_SPACES,
      g_param_spec_boolean("convert-spaces", NULL, NULL, FALSE,
                           G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_SPACE_CHARACTER,
      g_param_spec_char("space-character", NULL, NULL, ' ', '~', '_',
                        G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_COUNTER_START,
      g_param_spec_ulong("counter-start", NULL, NULL, 0, G_MAXULONG, 1,
                         G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_COUNTER_N_DIGITS,
      g_param_spec_uint("counter-n-digits", NULL, NULL, 1, G_MAXUINT, 1,
                        G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_N_IMAGES,
      g_param_spec_uint("n-images", NULL, NULL, 1, G_MAXUINT, 1,
                        G_PARAM_WRITABLE));
}

GQuark eom_uc_error_quark(void) {
  static GQuark q = 0;
  if (q == 0) q = g_quark_from_static_string("eom-uri-converter-error-quark");
...
static void eom_uri_converter_set_property(GObject *object, guint property_id,
  ...
  switch (property_id) {
    ...
    case PROP_COUNTER_START: {
      ...
      new_n_digits = ceil(
          log10(priv->counter_start + pow(10, priv->counter_n_digits) - 1));
    ...
    case PROP_COUNTER_N_DIGITS:
      priv->counter_n_digits =
          ceil(MIN(log10(G_MAXULONG), g_value_get_uint(value)));
      break;

    case PROP_N_IMAGES:
      priv->counter_n_digits =
          ceil(MIN(log10(G_MAXULONG),
...
/* parser states */
enum { PARSER_NONE, PARSER_STRING, PARSER_TOKEN };

static EomUCToken *create_token_string(const char *string, int substr_start,
                                       int substr_len) {
  char *start_byte;
  char *end_byte;
  int n_bytes;
  ...
  token->data.string =
      g_utf8_strncpy(token->data.string, start_byte, substr_len);

  return token;
}

static EomUCToken *create_token_counter(int start_counter) {
  EomUCToken *token;
...
static EomUCToken *create_token_other(EomUCType type) {
  EomUCToken *token;
...
static GList *eom_uri_converter_parse_string(EomURIConverter *conv,
                                             const char *string) {
  EomURIConverterPrivate *priv;
  GList *list = NULL;
  gulong len;
...
static GFile *get_file_directory(EomURIConverter *conv, EomImage *image) {
  GFile *file = NULL;
  EomURIConverterPrivate *priv;
...
static void split_filename(GFile *file, char **name, char **suffix) {
  char *basename;
  char *suffix_start;
  guint len;
...
static GString *append_filename(GString *str, EomImage *img) {
...
static GString *append_counter(GString *str, gulong counter,
                               EomURIConverter *conv) {
  EomURIConverterPrivate *priv;
...
static void build_absolute_file(EomURIConverter *conv, EomImage *image,
                                GString *str, /* input  */
                                GFile **file,
                                GdkPixbufFormat **format) /* output */
{
  GFile *dir_file;
  EomURIConverterPrivate *priv;
  ...
  *file = NULL;
  ...
  if (priv->img_format == NULL) {
    ...
    if (format != NULL) *format = eom_pixbuf_get_format_by_suffix(old_suffix);
    ...
    if (format != NULL) *format = priv->img_format;
  }
...
static GString *replace_remove_chars(GString *str, gboolean convert_spaces,
                                     gunichar space_char) {
  GString *result;
  guint len;
  char *s;
  ...
  if (!g_utf8_validate(str->str, -1, NULL)) return NULL;
 ...
 */
gboolean eom_uri_converter_do(EomURIConverter *conv, EomImage *image,
  ...
  *file = NULL;
  ...
  repl_str =
      replace_remove_chars(str, priv->convert_spaces, priv->space_character);
...
char *eom_uri_converter_preview(const char *format_str, EomImage *img,
                                GdkPixbufFormat *format, gulong counter,
                                guint n_images, gboolean convert_spaces,
                                gunichar space_char) {
  GString *str;
  GString *repl_str;
  guint n_digits;
  ...
  if (!g_utf8_validate(format_str, -1, NULL)) return NULL;
  ...
  for (i = 0; i < len; i++, s = g_utf8_next_char(s)) {
    ...
    if (token_next) {
      if (c == 'f') {
        ...
        g_string_append_printf(str, "%.*lu", n_digits, counter);
      }
...
gboolean eom_uri_converter_check(EomURIConverter *converter, GList *img_list,
                                 GError **error) {
  GList *it;
  ...
  for (it = img_list; it != NULL; it = it->next) {
    ...
    result = eom_uri_converter_do(converter, EOM_IMAGE(it->data), &file, NULL,
                                  &conv_error);
  ...
  if (!all_different) {
    g_set_error(error, EOM_UC_ERROR, EOM_UC_ERROR_EQUAL_FILENAMES,
