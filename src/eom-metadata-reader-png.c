...
#if 0
#define IS_FINISHED(priv) (priv->icc_chunk != NULL && priv->xmp_chunk != NULL)
#endif
...
static void eom_metadata_reader_png_init_emr_iface(gpointer g_iface,
                                                   gpointer iface_data);

G_DEFINE_TYPE_WITH_CODE(
    EomMetadataReaderPng, eom_metadata_reader_png, G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE(EOM_TYPE_METADATA_READER,
                          eom_metadata_reader_png_init_emr_iface)
        G_ADD_PRIVATE(EomMetadataReaderPng))

static void eom_metadata_reader_png_dispose(GObject *object) {
...
static void eom_metadata_reader_png_init(EomMetadataReaderPng *emr) {
...
static void eom_metadata_reader_png_class_init(
    EomMetadataReaderPngClass *klass) {
  GObjectClass *object_class = (GObjectClass *)klass;

  object_class->dispose = eom_metadata_reader_png_dispose;
}

static gboolean eom_metadata_reader_png_finished(EomMetadataReaderPng *emr) {
  g_return_val_if_fail(EOM_IS_METADATA_READER_PNG(emr), TRUE);

  return (emr->priv->state == EMR_FINISHED);
}

static void eom_metadata_reader_png_get_next_block(
    EomMetadataReaderPngPrivate *priv, guchar *chunk, int *i, const guchar *buf,
    int len, EomMetadataReaderPngState state) {
  if (*i + priv->size < len) {
...
static void eom_metadata_reader_png_consume(EomMetadataReaderPng *emr,
                                            const guchar *buf, guint len) {
  EomMetadataReaderPngPrivate *priv;
  int i;
  guint32 chunk_crc;
  ...
  for (i = 0; (i < len) && (priv->state != EMR_FINISHED); i++) {
    switch (priv->state) {
      case EMR_READ_MAGIC:
        ...
        if (priv->sub_step < 8 && (gchar)buf[i] == PNGMAGIC[priv->sub_step]) {
          if (priv->sub_step == 7) priv->state = EMR_READ_SIZE_HIGH_HIGH_BYTE;
          priv->sub_step++;
        } else {
      ...
      case EMR_READ_CHUNK_NAME:
        ...
        if (priv->sub_step > 3) g_assert_not_reached();

        priv->chunk_name[priv->sub_step] = buf[i];

        if (priv->sub_step++ != 3) break;

        if (G_UNLIKELY(!priv->hasIHDR)) {
          /* IHDR should be the first chunk in a PNG */
          if (priv->size == 13 && memcmp(priv->chunk_name, "IHDR", 4) == 0) {
            priv->hasIHDR = TRUE;
          } else {
      ...
      case EMR_SKIP_BYTES:
        ...
        eom_debug_message(DEBUG_IMAGE_DATA, "Skip bytes: %" G_GSIZE_FORMAT,
                          priv->size);
      ...
      case EMR_CHECK_CRC:
        ...
        if (priv->sub_step == 0) priv->target_crc = 0;
        ...
        eom_debug_message(DEBUG_IMAGE_DATA,
                          "Checking CRC: Chunk: 0x%X - Target: 0x%X", chunk_crc,
                          priv->target_crc);
      ...
      case EMR_READ_XMP_ITXT:
         ...
         * an XMP packet */
        eom_debug_message(DEBUG_IMAGE_DATA,
                          "Read XMP Chunk - size: %" G_GSIZE_FORMAT,
                          priv->size);
        ...
        eom_metadata_reader_png_get_next_block(priv, priv->xmp_chunk, &i, buf,
                                               len, EMR_READ_XMP_ITXT);
      ...
      case EMR_READ_ICCP:
        ...
        eom_debug_message(DEBUG_IMAGE_DATA,
                          "Read ICC Chunk - size: %" G_GSIZE_FORMAT,
                          priv->size);
        ...
        eom_metadata_reader_png_get_next_block(priv, priv->icc_chunk, &i, buf,
                                               len, EMR_READ_ICCP);
        break;
      case EMR_READ_SRGB:
        /* Extract the sRGB chunk. Marks the image data as
         * being in sRGB colorspace. */
        eom_debug_message(DEBUG_IMAGE_DATA, "Read sRGB Chunk - value: %u",
                          *(buf + i));
        ...
        eom_metadata_reader_png_get_next_block(priv, priv->sRGB_chunk, &i, buf,
                                               len, EMR_READ_SRGB);
        break;
      case EMR_READ_CHRM:
        ...
        eom_debug_message(DEBUG_IMAGE_DATA,
                          "Read cHRM Chunk - size: %" G_GSIZE_FORMAT,
                          priv->size);
        ...
        eom_metadata_reader_png_get_next_block(priv, priv->cHRM_chunk, &i, buf,
                                               len, EMR_READ_ICCP);
        break;
      case EMR_READ_GAMA:
         ...
         * image's gamma value */
        eom_debug_message(DEBUG_IMAGE_DATA,
                          "Read gAMA-Chunk - size: %" G_GSIZE_FORMAT,
                          priv->size);
        ...
        eom_metadata_reader_png_get_next_block(priv, priv->gAMA_chunk, &i, buf,
                                               len, EMR_READ_ICCP);
        break;
      default:
...
static gpointer eom_metadata_reader_png_get_xmp_data(
    EomMetadataReaderPng *emr) {
  EomMetadataReaderPngPrivate *priv;
  XmpPtr xmp = NULL;
...
#define EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(chunk, offset, divider)     \
  (double)(GUINT32_FROM_BE(*((guint32 *)((chunk) + ((offset)*4)))) / \
           (double)(divider))
 ...
 */
static gboolean _chrm_matches_srgb(const cmsCIExyY *whitepoint,
...
static gpointer eom_metadata_reader_png_get_icc_profile(
    EomMetadataReaderPng *emr) {
  EomMetadataReaderPngPrivate *priv;
  ...
  if (priv->icc_chunk) {
    ...
    while (*((guchar *)priv->icc_chunk + offset) != '\0') offset++;
    /* Ensure the compression method (deflate) */
    if (*((guchar *)priv->icc_chunk + (++offset)) != '\0') return NULL;
    ...
    do {
      if (zstr.avail_out == 0) {
        ...
        if (G_UNLIKELY(new_size > EOM_ICC_INFLATE_BUFFER_LIMIT)) {
          ...
          g_free(outbuf);
          eom_debug_message(DEBUG_IMAGE_DATA,
                            "ICC profile is too large. Ignoring.");
          return NULL;
        }
    ...
    if (G_UNLIKELY(z_ret != Z_STREAM_END)) {
      eom_debug_message(DEBUG_IMAGE_DATA,
                        "Error while inflating ICC profile: %s (%d)", zstr.msg,
                        z_ret);
      inflateEnd(&zstr);
      g_free(outbuf);
      return NULL;
    ...
    eom_debug_message(DEBUG_LCMS, "PNG has %s ICC profile",
  ...
  if (!profile && priv->cHRM_chunk && priv->gAMA_chunk) {
    ...
    whitepoint.x =
        EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(priv->cHRM_chunk, 0, 100000);
    whitepoint.y =
        EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(priv->cHRM_chunk, 1, 100000);

    primaries.Red.x =
        EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(priv->cHRM_chunk, 2, 100000);
    primaries.Red.y =
        EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(priv->cHRM_chunk, 3, 100000);
    primaries.Green.x =
        EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(priv->cHRM_chunk, 4, 100000);
    primaries.Green.y =
        EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(priv->cHRM_chunk, 5, 100000);
    primaries.Blue.x =
        EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(priv->cHRM_chunk, 6, 100000);
    primaries.Blue.y =
        EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(priv->cHRM_chunk, 7, 100000);

    whitepoint.Y = primaries.Red.Y = primaries.Green.Y = primaries.Blue.Y = 1.0;

    gammaValue = (double)1.0 /
                 EXTRACT_DOUBLE_UINT_BLOCK_OFFSET(priv->gAMA_chunk, 0, 100000);
...
static void eom_metadata_reader_png_init_emr_iface(gpointer g_iface,
  ...
  iface->finished =
      (gboolean(*)(EomMetadataReader * self)) eom_metadata_reader_png_finished;
#if defined(HAVE_LCMS) && defined(GDK_WINDOWING_X11)
  iface->get_icc_profile = (cmsHPROFILE(*)(EomMetadataReader * self))
      eom_metadata_reader_png_get_icc_profile;
#endif
#ifdef HAVE_EXEMPI
  iface->get_xmp_ptr = (gpointer(*)(EomMetadataReader * self))
