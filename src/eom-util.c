...
void eom_util_show_help(const gchar *section, GtkWindow *parent) {
  GError *error = NULL;
  gchar *uri = NULL;

  if (section) uri = g_strdup_printf("help:eom/%s", section);
  ...
  if (error) {
    ...
    dialog =
        gtk_message_dialog_new(parent, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK,
...
gchar *eom_util_make_valid_utf8(const gchar *str) {
  GString *string;
...
GSList *eom_util_string_list_to_file_list(GSList *string_list) {
  ...
  for (it = string_list; it != NULL; it = it->next) {
    ...
    file_list = g_slist_prepend(file_list, g_file_new_for_uri(uri_str));
  }
...
GSList *eom_util_strings_to_file_list(gchar **strings) {
  ...
  for (i = 0; strings[i]; i++) {
    file_list = g_slist_prepend(file_list, g_file_new_for_uri(strings[i]));
  }
...
static gboolean ensure_dir_exists(const char *dir) {
  ...
  if (g_mkdir_with_parents(dir, 0700) == 0) return TRUE;
...
const gchar *eom_util_dot_dir(void) {
  if (dot_dir == NULL) {
    ...
    if (G_UNLIKELY(!exists)) {
      ...
      if (!printed_warning) {
        g_warning(
            "EOM could not save some of your preferences in its settings "
            "directory due to a file with the same name (%s) blocking its "
 ...
 **/
char *eom_util_filename_get_extension(const char *filename) {
  char *begin, *begin2;
  ...
  if (begin && begin != filename) {
    if (strcmp(begin, ".gz") == 0 || strcmp(begin, ".bz2") == 0 ||
        strcmp(begin, ".sit") == 0 || strcmp(begin, ".Z") == 0) {
      begin2 = begin - 1;
      while (begin2 > filename && *begin2 != '.') {
        begin2--;
      }
      if (begin2 != filename) {
        begin = begin2;
...
static void _eom_util_show_file_in_filemanager_fallback(GFile *file,
                                                        GtkWindow *toplevel) {
  gchar *uri = NULL;
  GError *error = NULL;
  ...
  if (g_file_query_file_type(file, 0, NULL) == G_FILE_TYPE_DIRECTORY) {
    ...
    if (G_LIKELY(parent_file)) uri = g_file_get_uri(parent_file);
...
void eom_util_show_file_in_filemanager(GFile *file, GtkWindow *toplevel) {
  GDBusProxy *proxy;
  gboolean done = FALSE;
  ...
  proxy = g_dbus_proxy_new_for_bus_sync(
      ...
      "org.freedesktop.FileManager1", NULL, NULL);
  ...
  if (proxy) {
    ...
    result = g_dbus_proxy_call_sync(proxy, "ShowItems", params,
  ...
  if (!done) _eom_util_show_file_in_filemanager_fallback(file, toplevel);
}

gboolean eom_notebook_scroll_event_cb(GtkWidget *widget, GdkEventScroll *event)
