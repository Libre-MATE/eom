...
static void init_transform_info(EomImage *image, jpeg_transform_info *info) {
...
static gboolean _save_jpeg_as_jpeg(EomImage *image, const char *file,
  ...
  if (priv->exif_chunk != NULL) {
    jpeg_write_marker(&dstinfo, JPEG_APP0 + 1, priv->exif_chunk,
                      priv->exif_chunk_len);
  }
#endif
...
static gboolean _save_any_as_jpeg(EomImage *image, const char *file,
                                  EomImageSaveInfo *source,
                                  EomImageSaveInfo *target, GError **error) {
  EomImagePrivate *priv;
  GdkPixbuf *pixbuf;
  ...
  if (outfile == NULL) {
    g_set_error(error, /* FIXME: Better error message */
                GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_INSUFFICIENT_MEMORY,
                _("Couldn't create temporary file for saving: %s"), file);
    return FALSE;
  }
...
#else
  if (priv->exif_chunk != NULL) {
    jpeg_write_marker(&cinfo, JPEG_APP0 + 1, priv->exif_chunk,
                      priv->exif_chunk_len);
  }
#endif
  /* FIXME: Consider IPTC data too */
