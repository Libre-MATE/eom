...
static void eom_plugin_engine_dispose(GObject *object) {
...
static void eom_plugin_engine_class_init(EomPluginEngineClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->dispose = eom_plugin_engine_dispose;
}

static void eom_plugin_engine_init(EomPluginEngine *engine) {
...
EomPluginEngine *eom_plugin_engine_new(void) {
  ...
  if (g_irepository_require_private(g_irepository_get_default(), private_path,
                                    "Eom", "1.0", 0, &error) == NULL) {
  ...
  user_plugin_path =
      g_build_filename(eom_util_dot_dir(), USER_EOM_PLUGINS_LOCATION, NULL);

  peas_engine_add_search_path(PEAS_ENGINE(engine), user_plugin_path,
                              user_plugin_path);

  peas_engine_add_search_path(PEAS_ENGINE(engine), EOM_PLUGIN_DIR,
                              EOM_PLUGIN_DIR);

  g_settings_bind(engine->priv->plugins_settings,
                  EOM_CONF_PLUGINS_ACTIVE_PLUGINS, engine, "loaded-plugins",
