...
static void eom_statusbar_class_init(EomStatusbarClass *klass) { /* empty */ }

static void eom_statusbar_init(EomStatusbar *statusbar) {
  ...
  gtk_box_pack_end(GTK_BOX(statusbar), priv->img_num_label, FALSE, TRUE, 0);

  vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);

  gtk_box_pack_end(GTK_BOX(statusbar), vbox, FALSE, FALSE, 2);

  statusbar->priv->progressbar = gtk_progress_bar_new();

  gtk_box_pack_end(GTK_BOX(vbox), priv->progressbar, TRUE, TRUE, 2);
...
void eom_statusbar_set_image_number(EomStatusbar *statusbar, gint num,
  ...
  if (G_UNLIKELY(num <= 0 || tot <= 0)) return;
