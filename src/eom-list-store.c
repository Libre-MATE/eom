...
static void foreach_monitors_free(gpointer data, gpointer user_data) {
  g_file_monitor_cancel(G_FILE_MONITOR(data));
}

static void eom_list_store_dispose(GObject *object) {
  EomListStore *store = EOM_LIST_STORE(object);

  g_list_foreach(store->priv->monitors, foreach_monitors_free, NULL);
...
static void eom_list_store_class_init(EomListStoreClass *klass) {
...
static gint eom_list_store_compare_func(GtkTreeModel *model, GtkTreeIter *a,
...
static GdkPixbuf *eom_list_store_get_icon(const gchar *icon_name) {
  GError *error = NULL;
  ...
  pixbuf = gtk_icon_theme_load_icon(icon_theme, icon_name,
                                    EOM_LIST_STORE_THUMB_SIZE, 0, &error);
...
static void eom_list_store_init(EomListStore *self) {
  ...
  gtk_tree_sortable_set_default_sort_func(
      GTK_TREE_SORTABLE(self), eom_list_store_compare_func, NULL, NULL);
 ...
 */
static gboolean is_file_in_list_store(EomListStore *store,
  ...
  do {
    gtk_tree_model_get(GTK_TREE_MODEL(store), &iter, EOM_LIST_STORE_EOM_IMAGE,
                       &image, -1);
    if (!image) continue;
...
static gboolean is_file_in_list_store_file(EomListStore *store, GFile *file,
                                           GtkTreeIter *iter_found) {
  gchar *uri_str;
  gboolean result;
...
static void eom_job_thumbnail_cb(EomJobThumbnail *job, gpointer data) {
  EomListStore *store;
  GtkTreeIter iter;
  EomImage *image;
  GdkPixbuf *thumbnail;
  ...
  if (is_file_in_list_store_file(store, file, &iter)) {
    ...
    gtk_list_store_set(GTK_LIST_STORE(store), &iter, EOM_LIST_STORE_THUMBNAIL,
                       thumbnail, EOM_LIST_STORE_THUMB_SET, TRUE,
                       EOM_LIST_STORE_EOM_JOB, NULL, -1);
...
static void on_image_changed(EomImage *image, EomListStore *store) {
  GtkTreePath *path;
  GtkTreeIter iter;
  gint pos;
 ...
 **/
static void eom_list_store_remove(EomListStore *store, GtkTreeIter *iter) {
  EomImage *image;
...
void eom_list_store_append_image(EomListStore *store, EomImage *image) {
  ...
  g_signal_connect(image, "changed", G_CALLBACK(on_image_changed), store);

  gtk_list_store_append(GTK_LIST_STORE(store), &iter);
  gtk_list_store_set(GTK_LIST_STORE(store), &iter, EOM_LIST_STORE_EOM_IMAGE,
                     image, EOM_LIST_STORE_THUMBNAIL, store->priv->busy_image,
                     EOM_LIST_STORE_THUMB_SET, FALSE, -1);
}

static void eom_list_store_append_image_from_file(EomListStore *store,
                                                  GFile *file,
                                                  const gchar *caption) {
  EomImage *image;
  ...
  switch (event) {
    ...
    case G_FILE_MONITOR_EVENT_ATTRIBUTE_CHANGED:
      file_info = g_file_query_info(
          file, G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE, 0, NULL, NULL);
      if (file_info == NULL) {
        break;
      }
      mimetype = g_file_info_get_content_type(file_info);
 ...
 * list.
 */
static void directory_visit(GFile *directory, GFileInfo *children_info,
                            EomListStore *store) {
  GFile *child;
  ...
  if (!g_str_has_prefix(name, ".")) {
    /* We support opening any image type, so let eom to add any images in the
     * current directory to the store */
...
static void eom_list_store_append_directory(EomListStore *store, GFile *file,
                                            GFileType file_type) {
 ...
 *
 **/
void eom_list_store_add_files(EomListStore *store, GList *file_list) {
  GList *it;
  GFileInfo *file_info;
  GFileType file_type;
  ...
  gtk_tree_sortable_set_sort_column_id(
      GTK_TREE_SORTABLE(store), GTK_TREE_SORTABLE_UNSORTED_SORT_COLUMN_ID,
      GTK_SORT_ASCENDING);
  ...
  for (it = file_list; it != NULL; it = it->next) {
    ...
    if (file_type == G_FILE_TYPE_DIRECTORY) {
      ...
      file_info = g_file_query_info(file, G_FILE_ATTRIBUTE_STANDARD_TYPE, 0,
                                    NULL, NULL);
      ...
      if (file_type == G_FILE_TYPE_DIRECTORY) {
        ...
        if (!is_file_in_list_store_file(store, initial_file, &iter)) {
  ...
  if (initial_file && is_file_in_list_store_file(store, initial_file, &iter)) {
 ...
 **/
void eom_list_store_remove_image(EomListStore *store, EomImage *image) {
  GtkTreeIter iter;
  GFile *file;
...
GtkListStore *eom_list_store_new_from_glist(GList *list) {
  ...
  for (it = list; it != NULL; it = it->next) {
    eom_list_store_append_image(EOM_LIST_STORE(store), EOM_IMAGE(it->data));
  }
 ...
 **/
gint eom_list_store_get_pos_by_image(EomListStore *store, EomImage *image) {
  GtkTreeIter iter;
  gint pos = -1;
  GFile *file;
 ...
 *
 **/
EomImage *eom_list_store_get_image_by_pos(EomListStore *store, gint pos) {
  EomImage *image = NULL;
  GtkTreeIter iter;
 ...
 **/
gint eom_list_store_get_pos_by_iter(EomListStore *store, GtkTreeIter *iter) {
  gint *indices;
  GtkTreePath *path;
  gint pos;
...
static void eom_list_store_remove_thumbnail_job(EomListStore *store,
                                                GtkTreeIter *iter) {
  EomJob *job;

  gtk_tree_model_get(GTK_TREE_MODEL(store), iter, EOM_LIST_STORE_EOM_JOB, &job,
                     -1);

  if (job != NULL) {
    g_mutex_lock(&store->priv->mutex);
    eom_job_queue_remove_job(job);
    gtk_list_store_set(GTK_LIST_STORE(store), iter, EOM_LIST_STORE_EOM_JOB,
                       NULL, -1);
    g_mutex_unlock(&store->priv->mutex);
  }
}

static void eom_list_store_add_thumbnail_job(EomListStore *store,
                                             GtkTreeIter *iter) {
  EomImage *image;
  EomJob *job;

  gtk_tree_model_get(GTK_TREE_MODEL(store), iter, EOM_LIST_STORE_EOM_IMAGE,
                     &image, EOM_LIST_STORE_EOM_JOB, &job, -1);
  ...
  g_signal_connect(job, "finished", G_CALLBACK(eom_job_thumbnail_cb), store);

  g_mutex_lock(&store->priv->mutex);
  gtk_list_store_set(GTK_LIST_STORE(store), iter, EOM_LIST_STORE_EOM_JOB, job,
                     -1);
 ...
 *
 **/
void eom_list_store_thumbnail_set(EomListStore *store, GtkTreeIter *iter) {
  gboolean thumb_set = FALSE;

  gtk_tree_model_get(GTK_TREE_MODEL(store), iter, EOM_LIST_STORE_THUMB_SET,
                     &thumb_set, -1);
 ...
 *
 **/
void eom_list_store_thumbnail_unset(EomListStore *store, GtkTreeIter *iter) {
  EomImage *image;
  ...
  gtk_list_store_set(GTK_LIST_STORE(store), iter, EOM_LIST_STORE_THUMBNAIL,
                     store->priv->busy_image, EOM_LIST_STORE_THUMB_SET, FALSE,
                     -1);
}
 ...
 *
 **/
void eom_list_store_thumbnail_refresh(EomListStore *store, GtkTreeIter *iter) {
  eom_list_store_remove_thumbnail_job(store, iter);
  eom_list_store_add_thumbnail_job(store, iter);
}
