...
static GdkPixbufFormat *get_selected_format(GtkComboBox *combobox) {
...
static gboolean update_preview(gpointer user_data) {
  ...
  counter_start =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(data->counter_spin));

  format = get_selected_format(GTK_COMBO_BOX(data->format_combobox));

  if (token_str != NULL) {
    /* generate preview filename */
    preview_str = eom_uri_converter_preview(
        token_str, data->image, format, (counter_start + data->nth_image),
        data->n_images, convert_spaces, '_' /* FIXME: make this editable */);
  }
...
static void request_preview_update(GtkWidget *dlg) {
  SaveAsData *data;

  data = g_object_get_data(G_OBJECT(dlg), "data");
  g_assert(data != NULL);

  if (data->idle_id != 0) return;

  data->idle_id = g_idle_add(update_preview, dlg);
}

static void on_format_combobox_changed(GtkComboBox *widget, gpointer data) {
  request_preview_update(GTK_WIDGET(data));
}

static void on_token_entry_changed(GtkWidget *widget, gpointer user_data) {
  SaveAsData *data;
...
static void on_replace_spaces_check_clicked(GtkWidget *widget, gpointer data) {
  request_preview_update(GTK_WIDGET(data));
}

static void on_counter_spin_changed(GtkWidget *widget, gpointer data) {
  request_preview_update(GTK_WIDGET(data));
}

static void prepare_format_combobox(SaveAsData *data) {
  ...
  for (it = formats; it != NULL; it = it->next) {
    ...
    gtk_list_store_set(store, &iter, 0, gdk_pixbuf_format_get_name(f), 1, f,
                       -1);
  }
...
static void destroy_data_cb(gpointer data) {
  SaveAsData *sd;

  sd = (SaveAsData *)data;

  if (sd->image != NULL) g_object_unref(sd->image);

  if (sd->idle_id != 0) g_source_remove(sd->idle_id);

  g_slice_free(SaveAsData, sd);
}

static void set_default_values(GtkWidget *dlg, GFile *base_file) {
  SaveAsData *sd;
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(sd->replace_spaces_check),
                               FALSE);
  if (base_file != NULL) {
    gtk_file_chooser_set_current_folder_file(GTK_FILE_CHOOSER(sd->dir_chooser),
                                             base_file, NULL);
  }
...
GtkWidget *eom_save_as_dialog_new(GtkWindow *main, GList *images,
                                  GFile *base_file) {
  GtkBuilder *xml;
  GtkWidget *dlg;
  SaveAsData *data;
  ...
  xml = gtk_builder_new_from_resource(
      "/org/mate/eom/ui/eom-multiple-save-as-dialog.ui");
#ifdef ENABLE_NLS
  gtk_builder_set_translation_domain(xml, GETTEXT_PACKAGE);
#endif /* ENABLE_NLS */

  dlg = GTK_WIDGET(
      g_object_ref(gtk_builder_get_object(xml, "eom_multiple_save_as_dialog")));
  ...
  data->dir_chooser = GTK_WIDGET(gtk_builder_get_object(xml, "dir_chooser"));
  data->token_entry = GTK_WIDGET(gtk_builder_get_object(xml, "token_entry"));
  data->replace_spaces_check =
      GTK_WIDGET(gtk_builder_get_object(xml, "replace_spaces_check"));
  data->counter_spin = GTK_WIDGET(gtk_builder_get_object(xml, "counter_spin"));
  data->preview_label =
      GTK_WIDGET(gtk_builder_get_object(xml, "preview_label"));
  data->format_combobox =
      GTK_WIDGET(gtk_builder_get_object(xml, "format_combobox"));

  /* init preview information */
  data->idle_id = 0;
  data->n_images = g_list_length(images);
  data->nth_image =
      (int)((float)data->n_images * rand() / (float)(RAND_MAX + 1.0));
  g_assert(data->nth_image >= 0 && data->nth_image < data->n_images);
  data->image =
      g_object_ref(EOM_IMAGE(g_list_nth_data(images, data->nth_image)));
  g_object_set_data_full(G_OBJECT(dlg), "data", data, destroy_data_cb);

  g_signal_connect(data->format_combobox, "changed",
                   G_CALLBACK(on_format_combobox_changed), dlg);

  g_signal_connect(data->token_entry, "changed",
                   G_CALLBACK(on_token_entry_changed), dlg);

  g_signal_connect(data->replace_spaces_check, "toggled",
                   G_CALLBACK(on_replace_spaces_check_clicked), dlg);

  g_signal_connect(data->counter_spin, "changed",
                   G_CALLBACK(on_counter_spin_changed), dlg);
...
EomURIConverter *eom_save_as_dialog_get_converter(GtkWidget *dlg) {
  ...
  g_object_set(G_OBJECT(conv), "convert-spaces", convert_spaces,
               "space-character", '_', "counter-start", counter_start,
               "n-images", data->n_images, NULL);
