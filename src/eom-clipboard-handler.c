...
enum { PROP_0, PROP_PIXBUF, PROP_URI };

enum { TARGET_PIXBUF, TARGET_TEXT, TARGET_URI };
...
G_DEFINE_TYPE_WITH_PRIVATE(EomClipboardHandler, eom_clipboard_handler,
                           G_TYPE_INITIALLY_UNOWNED)

static GdkPixbuf *eom_clipboard_handler_get_pixbuf(
...
static const gchar *eom_clipboard_handler_get_uri(
    EomClipboardHandler *handler) {
  g_return_val_if_fail(EOM_IS_CLIPBOARD_HANDLER(handler), NULL);
...
static void eom_clipboard_handler_set_pixbuf(EomClipboardHandler *handler,
                                             GdkPixbuf *pixbuf) {
  g_return_if_fail(EOM_IS_CLIPBOARD_HANDLER(handler));
  g_return_if_fail(pixbuf == NULL || GDK_IS_PIXBUF(pixbuf));

  if (handler->priv->pixbuf == pixbuf) return;

  if (handler->priv->pixbuf) g_object_unref(handler->priv->pixbuf);
...
static void eom_clipboard_handler_set_uri(EomClipboardHandler *handler,
                                          const gchar *uri) {
  g_return_if_fail(EOM_IS_CLIPBOARD_HANDLER(handler));

  if (handler->priv->uri != NULL) g_free(handler->priv->uri);

  handler->priv->uri = g_strdup(uri);
  g_object_notify(G_OBJECT(handler), "uri");
}

static void eom_clipboard_handler_get_property(GObject *object,
  ...
  switch (property_id) {
    case PROP_PIXBUF:
      g_value_set_object(value, eom_clipboard_handler_get_pixbuf(handler));
      break;
    case PROP_URI:
      g_value_set_string(value, eom_clipboard_handler_get_uri(handler));
      break;
    default:
...
static void eom_clipboard_handler_set_property(GObject *object,
                                               guint property_id,
                                               const GValue *value,
                                               GParamSpec *pspec) {
  EomClipboardHandler *handler;
...
static void eom_clipboard_handler_dispose(GObject *obj) {
...
static void eom_clipboard_handler_init(EomClipboardHandler *handler) {
  handler->priv = eom_clipboard_handler_get_instance_private(handler);
}

static void eom_clipboard_handler_class_init(EomClipboardHandlerClass *klass) {
...
EomClipboardHandler *eom_clipboard_handler_new(EomImage *img) {
  ...
  obj = g_object_new(EOM_TYPE_CLIPBOARD_HANDLER, "pixbuf", pbuf, "uri", uri,
                     NULL);
  g_free(uri);
  g_object_unref(file);
  g_object_unref(pbuf);
...
static void eom_clipboard_handler_get_func(GtkClipboard *clipboard,
  ...
  switch (info) {
    ...
    case TARGET_TEXT: {
      gtk_selection_data_set_text(selection,
                                  eom_clipboard_handler_get_uri(handler), -1);
      break;
    }
    case TARGET_URI: {
      gchar *uris[2];
...
static void eom_clipboard_handler_clear_func(GtkClipboard *clipboard,
                                             gpointer owner) {
  g_return_if_fail(EOM_IS_CLIPBOARD_HANDLER(owner));
...
void eom_clipboard_handler_copy_to_clipboard(EomClipboardHandler *handler,
  ...
  if (n_targets > 0) {
    set = gtk_clipboard_set_with_owner(
        clipboard, targets, n_targets, eom_clipboard_handler_get_func,
        eom_clipboard_handler_clear_func, G_OBJECT(handler));
  }
