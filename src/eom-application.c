...
G_DEFINE_TYPE_WITH_PRIVATE(EomApplication, eom_application,
                           GTK_TYPE_APPLICATION);

static void eom_application_activate(GApplication *application) {
  eom_application_open_window(EOM_APPLICATION(application), GDK_CURRENT_TIME,
                              EOM_APPLICATION(application)->priv->flags, NULL);
}

static void eom_application_open(GApplication *application, GFile **files,
                                 gint n_files, const gchar *hint) {
  GSList *list = NULL;

  while (n_files--) list = g_slist_prepend(list, files[n_files]);

  eom_application_open_file_list(
      EOM_APPLICATION(application), list, GDK_CURRENT_TIME,
      EOM_APPLICATION(application)->priv->flags, NULL);
}

static void eom_application_finalize(GObject *object) {
...
static void eom_application_add_platform_data(GApplication *application,
                                              GVariantBuilder *builder) {
  EomApplication *app = EOM_APPLICATION(application);

  G_APPLICATION_CLASS(eom_application_parent_class)
      ->add_platform_data(application, builder);

  if (app->priv->flags) {
    g_variant_builder_add(builder, "{sv}", "eom-application-startup-flags",
                          g_variant_new_byte(app->priv->flags));
  }
}

static void eom_application_before_emit(GApplication *application,
...
static void eom_application_class_init(
    EomApplicationClass *eom_application_class) {
...
static void on_extension_added(PeasExtensionSet *set, PeasPluginInfo *info,
                               PeasExtension *exten, EomApplication *app) {
...
static void eom_application_init(EomApplication *eom_application) {
  ...
  if (G_LIKELY(dot_dir != NULL))
    priv->toolbars_file = g_build_filename(dot_dir, "eom_toolbar.xml", NULL);
  ...
  priv->extensions = peas_extension_set_new(
      PEAS_ENGINE(priv->plugin_engine), EOM_TYPE_APPLICATION_ACTIVATABLE, "app",
      EOM_APPLICATION(eom_application), NULL);
...
EomApplication *eom_application_get_instance(void) {
  ...
  if (!instance) {
    instance = EOM_APPLICATION(g_object_new(
        EOM_TYPE_APPLICATION, "application-id", APPLICATION_SERVICE_NAME,
...
static EomWindow *eom_application_get_empty_window(
...
gboolean eom_application_open_window(EomApplication *application,
                                     guint32 timestamp, EomStartupFlags flags,
                                     GError **error) {
...
static EomWindow *eom_application_get_file_window(EomApplication *application,
                                                  GFile *file) {
...
static void eom_application_show_window(EomWindow *window, gpointer user_data) {
  if (!gtk_widget_get_realized(GTK_WIDGET(window)))
    gtk_widget_realize(GTK_WIDGET(window));

  gtk_window_present_with_time(GTK_WINDOW(window), GPOINTER_TO_UINT(user_data));
}
...
gboolean eom_application_open_file_list(EomApplication *application,
  ...
  if (file_list != NULL)
    new_window =
        eom_application_get_file_window(application, (GFile *)file_list->data);
  ...
  g_signal_connect(new_window, "prepared",
...
static void eom_application_load_accelerators(void) {
  gchar *accelfile =
      g_build_filename(g_get_user_config_dir(), "mate", "accels", "eom", NULL);
