...
/* Properties */
enum { PROP_0, PROP_UNSAVED_IMAGES };

/* Mode */
enum { SINGLE_IMG_MODE, MULTIPLE_IMGS_MODE };
...
#define GET_MODE(priv)                                                      \
  (((priv->unsaved_images != NULL) && (priv->unsaved_images->next == NULL)) \
       ? SINGLE_IMG_MODE                                                    \
       : MULTIPLE_IMGS_MODE)

#define IMAGE_COLUMN_HEIGHT 40

G_DEFINE_TYPE_WITH_PRIVATE(EomCloseConfirmationDialog,
                           eom_close_confirmation_dialog, GTK_TYPE_DIALOG)
...
static GdkPixbuf *eom_close_confirmation_dialog_get_icon(
  ...
  pixbuf = gtk_icon_theme_load_icon(icon_theme, icon_name, IMAGE_COLUMN_HEIGHT,
                                    0, &error);
...
static GdkPixbuf *get_nothumb_pixbuf(void) {
  static GOnce nothumb_once = G_ONCE_INIT;
  g_once(&nothumb_once, (GThreadFunc)eom_close_confirmation_dialog_get_icon,
         "image-x-generic");
 ...
 */
static void response_cb(EomCloseConfirmationDialog *dlg, gint response_id,
                        gpointer data) {
  ...
  if (priv->selected_images != NULL) g_list_free(priv->selected_images);

  if (response_id == GTK_RESPONSE_YES) {
    if (GET_MODE(priv) == SINGLE_IMG_MODE) {
      priv->selected_images = g_list_copy(priv->unsaved_images);
    } else {
      g_return_if_fail(priv->list_store);

      priv->selected_images = get_selected_imgs(priv->list_store);
    }
  } else
    priv->selected_images = NULL;
}

static void add_buttons(EomCloseConfirmationDialog *dlg) {
  ...
  gtk_dialog_add_button(GTK_DIALOG(dlg), "gtk-save", GTK_RESPONSE_YES);

  gtk_dialog_set_default_response(GTK_DIALOG(dlg), GTK_RESPONSE_YES);
}

static void eom_close_confirmation_dialog_init(
...
static void eom_close_confirmation_dialog_finalize(GObject *object) {
  EomCloseConfirmationDialogPrivate *priv;

  priv = EOM_CLOSE_CONFIRMATION_DIALOG(object)->priv;

  if (priv->unsaved_images != NULL) g_list_free(priv->unsaved_images);
...
static void eom_close_confirmation_dialog_set_property(GObject *object,
                                                       guint prop_id,
                                                       const GValue *value,
                                                       GParamSpec *pspec) {
  EomCloseConfirmationDialog *dlg;
...
static void eom_close_confirmation_dialog_get_property(GObject *object,
                                                       guint prop_id,
                                                       GValue *value,
                                                       GParamSpec *pspec) {
  EomCloseConfirmationDialogPrivate *priv;
...
static void eom_close_confirmation_dialog_class_init(
    EomCloseConfirmationDialogClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_UNSAVED_IMAGES,
      g_param_spec_pointer("unsaved_images", "Unsaved Images",
...
static GList *get_selected_imgs(GtkTreeModel *store) {
  ...
  while (valid) {
    ...
    gtk_tree_model_get(store, &iter, SAVE_COLUMN, &to_save, IMG_COLUMN, &img,
                       -1);
    if (to_save) list = g_list_prepend(list, img);
...
GtkWidget *eom_close_confirmation_dialog_new(GtkWindow *parent,
  ...
  dlg = GTK_WIDGET(g_object_new(EOM_TYPE_CLOSE_CONFIRMATION_DIALOG,
                                "unsaved_images", unsaved_images, NULL));
...
GtkWidget *eom_close_confirmation_dialog_new_single(GtkWindow *parent,
  ...
  dlg = eom_close_confirmation_dialog_new(parent, unsaved_images);
...
static void build_single_img_dialog(EomCloseConfirmationDialog *dlg) {
  GtkWidget *hbox;
  GtkWidget *vbox;
  ...
  str = g_markup_printf_escaped(
      _("Save changes to image \"%s\" before closing?"), image_name);
...
static void populate_model(GtkTreeModel *store, GList *imgs) {
  GtkTreeIter iter;
  ...
  while (imgs != NULL) {
    ...
    if (buf) {
      ...
      buf_scaled =
          gdk_pixbuf_scale_simple(buf, (int)(ratio * (double)width),
                                  IMAGE_COLUMN_HEIGHT, GDK_INTERP_BILINEAR);
    } else
      buf_scaled = get_nothumb_pixbuf();

    gtk_list_store_append(GTK_LIST_STORE(store), &iter);
    gtk_list_store_set(GTK_LIST_STORE(store), &iter, SAVE_COLUMN, TRUE,
                       IMAGE_COLUMN, buf_scaled, NAME_COLUMN, name, IMG_COLUMN,
                       img, -1);
...
static GtkWidget *create_treeview(EomCloseConfirmationDialogPrivate *priv) {
  GtkListStore *store;
  GtkWidget *treeview;
  ...
  store = gtk_list_store_new(N_COLUMNS, G_TYPE_BOOLEAN, GDK_TYPE_PIXBUF,
  ...
  column = gtk_tree_view_column_new_with_attributes("Save?", renderer, "active",
                                                    SAVE_COLUMN, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview), column);

  renderer = gtk_cell_renderer_pixbuf_new();

  column = gtk_tree_view_column_new_with_attributes("Image", renderer, "pixbuf",
                                                    IMAGE_COLUMN, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview), column);
...
static void build_multiple_imgs_dialog(EomCloseConfirmationDialog *dlg) {
  EomCloseConfirmationDialogPrivate *priv;
  GtkWidget *hbox;
  GtkWidget *image;
  GtkWidget *vbox;
  ...
  str = g_strdup_printf(ngettext("There is %d image with unsaved changes. "
  ...
  select_label =
      gtk_label_new_with_mnemonic(_("S_elect the images you want to save:"));
