...
G_DEFINE_TYPE_WITH_PRIVATE(EomFileChooser, eom_file_chooser,
                           GTK_TYPE_FILE_CHOOSER_DIALOG)

static void eom_file_chooser_finalize(GObject *object) {
  EomFileChooserPrivate *priv;

  priv = EOM_FILE_CHOOSER(object)->priv;

  if (priv->thumb_factory != NULL) g_object_unref(priv->thumb_factory);

  (*G_OBJECT_CLASS(eom_file_chooser_parent_class)->finalize)(object);
}

static void eom_file_chooser_class_init(EomFileChooserClass *klass) {
  GObjectClass *object_class = (GObjectClass *)klass;

  object_class->finalize = eom_file_chooser_finalize;
}

static void eom_file_chooser_init(EomFileChooser *chooser) {
  chooser->priv = eom_file_chooser_get_instance_private(chooser);
}

static void response_cb(GtkDialog *dlg, gint id, gpointer data) {
  char *dir;
  ...
  if (id == GTK_RESPONSE_OK) {
    ...
    if (last_dir[action] != NULL) g_free(last_dir[action]);

    last_dir[action] = dir;
  }
}

static void save_response_cb(GtkDialog *dlg, gint id, gpointer data) {
  GFile *file;
  GdkPixbufFormat *format;

  if (id != GTK_RESPONSE_OK) return;
  ...
  if (!format || !gdk_pixbuf_format_is_writable(format)) {
    ...
    msg_dialog = gtk_message_dialog_new(
        GTK_WINDOW(dlg), GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK,
        _("File format is unknown or unsupported"));

    gtk_message_dialog_format_secondary_text(
        GTK_MESSAGE_DIALOG(msg_dialog), "%s\n%s",
        _("Eye of MATE could not determine a supported writable file format "
...
static void eom_file_chooser_add_filter(EomFileChooser *chooser) {
  GSList *it;
  GSList *formats;
...
static void set_preview_label(GtkWidget *label, const char *str) {
  if (str == NULL) {
 ...
 */
static void set_preview_pixbuf(EomFileChooser *chooser, GdkPixbuf *pixbuf,
                               goffset size) {
  EomFileChooserPrivate *priv;
  int bytes;
  int pixels;
...
static void update_preview_cb(GtkFileChooser *file_chooser, gpointer data) {
  ...
  if ((file_info != NULL) && (priv->thumb_factory != NULL) &&
    ...
    mtime = g_file_info_get_attribute_uint64(file_info,
                                             G_FILE_ATTRIBUTE_TIME_MODIFIED);
    thumb_path =
        mate_desktop_thumbnail_factory_lookup(priv->thumb_factory, uri, mtime);
...
static void eom_file_chooser_add_preview(GtkWidget *widget) {
  EomFileChooserPrivate *priv;
  GtkWidget *vbox;
...
GtkWidget *eom_file_chooser_new(GtkFileChooserAction action) {
  ...
  chooser = g_object_new(
      EOM_TYPE_FILE_CHOOSER, "action", action, "select-multiple",
      (action == GTK_FILE_CHOOSER_ACTION_OPEN), "local-only", FALSE, NULL);

  switch (action) {
    case GTK_FILE_CHOOSER_ACTION_OPEN:
      gtk_dialog_add_buttons(GTK_DIALOG(chooser), "gtk-cancel",
                             GTK_RESPONSE_CANCEL, "gtk-open", GTK_RESPONSE_OK,
                             NULL);
      title = _("Open Image");
      break;
    ...
    case GTK_FILE_CHOOSER_ACTION_SAVE:
      gtk_dialog_add_buttons(GTK_DIALOG(chooser), "gtk-cancel",
                             GTK_RESPONSE_CANCEL, "gtk-save", GTK_RESPONSE_OK,
                             NULL);
      title = _("Save Image");
      break;
  ...
  g_signal_connect(
      chooser, "response",
      G_CALLBACK((action == GTK_FILE_CHOOSER_ACTION_SAVE) ? save_response_cb
                                                          : response_cb),
      NULL);
