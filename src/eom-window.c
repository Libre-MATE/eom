...
enum { SIGNAL_PREPARED, SIGNAL_LAST };
...
static void eom_window_cmd_pause_slideshow(GtkAction *action,
                                           gpointer user_data);
...
static void eom_job_save_progress_cb(EomJobSave *job, float progress,
                                     gpointer data);
...
static void open_with_launch_application_cb(GtkAction *action,
...
static void eom_window_list_store_image_removed(GtkTreeModel *tree_model,
                                                GtkTreePath *path,
                                                gpointer user_data);
static void eom_window_set_wallpaper(EomWindow *window, const gchar *filename,
                                     const gchar *visible_filename);
...
static void disconnect_proxy_cb(GtkUIManager *manager, GtkAction *action,
                                GtkWidget *proxy, EomWindow *window);
static void eom_window_finish_saving(EomWindow *window);
static GAppInfo *get_appinfo_for_editor(EomWindow *window);

static GQuark eom_window_error_quark(void) {
  static GQuark q = 0;

  if (q == 0) q = g_quark_from_static_string("eom-window-error-quark");

  return q;
}

static void eom_window_set_collection_mode(EomWindow *window,
                                           EomWindowCollectionPos position,
                                           gboolean resizable) {
...
static void eom_window_can_save_changed_cb(GSettings *settings, gchar *key,
                                           gpointer user_data) {
  EomWindowPrivate *priv;
  EomWindow *window;
...
#if defined(HAVE_LCMS) && defined(GDK_WINDOWING_X11)
static cmsHPROFILE *eom_window_get_display_profile(GdkScreen *screen) {
  Display *dpy;
  Atom icc_atom, type;
  int format;
  gulong nitems;
  ...
  icc_atom = gdk_x11_get_xatom_by_name_for_display(
      gdk_screen_get_display(screen), atom_name);

  g_free(atom_name);

  result = XGetWindowProperty(
      dpy, GDK_WINDOW_XID(gdk_screen_get_root_window(screen)), icc_atom, 0,
...
static void update_image_pos(EomWindow *window) {
  EomWindowPrivate *priv;
  ...
  eom_statusbar_set_image_number(EOM_STATUSBAR(priv->statusbar), pos + 1,
                                 n_images);
}

static void update_status_bar(EomWindow *window) {
  EomWindowPrivate *priv;
  char *str = NULL;
  ...
  if (priv->image != NULL &&
    ...
    zoom = floor(100 * eom_scroll_view_get_zoom(EOM_SCROLL_VIEW(priv->view)) +
                 0.5);
...
static void eom_window_set_message_area(EomWindow *window,
                                        GtkWidget *message_area) {
  if (window->priv->message_area == message_area) return;
  ...
  gtk_box_pack_start(GTK_BOX(window->priv->cbox), window->priv->message_area,
                     FALSE, FALSE, 0);
...
static void update_action_groups_state(EomWindow *window) {
  ...
  action_sidebar =
      gtk_action_group_get_action(priv->actions_window, "ViewSidebar");

  action_fscreen =
      gtk_action_group_get_action(priv->actions_image, "ViewFullscreen");
  ...
  if (n_images == 0) {
    ...
    if (priv->flags & EOM_STARTUP_DISABLE_COLLECTION) {
      g_settings_set_boolean(priv->ui_settings, EOM_CONF_UI_IMAGE_COLLECTION,
                             FALSE);

      show_image_collection = FALSE;
    } else {
      show_image_collection = g_settings_get_boolean(
          priv->ui_settings, EOM_CONF_UI_IMAGE_COLLECTION);
    }

    show_image_collection = show_image_collection && n_images > 1 &&
...
static void update_selection_ui_visibility(EomWindow *window) {
  EomWindowPrivate *priv;
...
static gboolean add_file_to_recent_files(GFile *file) {
  ...
  gtk_recent_manager_add_full(gtk_recent_manager_get_default(), text_uri,
                              recent_data);
...
static void image_thumb_changed_cb(EomImage *image, gpointer data) {
  EomWindow *window;
  EomWindowPrivate *priv;
  GdkPixbuf *thumb;
...
static void file_changed_info_bar_response(GtkInfoBar *info_bar, gint response,
                                           EomWindow *window) {
...
static void image_file_changed_cb(EomImage *img, EomWindow *window) {
  GtkWidget *info_bar;
  gchar *text, *markup;
  GtkWidget *image;
  GtkWidget *label;
  ...
  if (window->priv->needs_reload_confirmation == FALSE) return;
  ...
  info_bar = gtk_info_bar_new_with_buttons(_("_Reload"), GTK_RESPONSE_YES,
  ...
  text = g_strdup_printf(
      _("The image \"%s\" has been modified by an external application."
  ...
  gtk_box_pack_start(
      GTK_BOX(gtk_info_bar_get_content_area(GTK_INFO_BAR(info_bar))), hbox,
      TRUE, TRUE, 0);
...
static void eom_window_display_image(EomWindow *window, EomImage *image) {
  EomWindowPrivate *priv;
  GFile *file;
  ...
  if (image != NULL) {
    ...
    g_signal_connect(image, "file-changed", G_CALLBACK(image_file_changed_cb),
                     window);
  ...
  g_idle_add_full(G_PRIORITY_LOW, (GSourceFunc)add_file_to_recent_files, file,
                  (GDestroyNotify)g_object_unref);

  eom_window_update_openwith_menu(window, image);
}

static void open_with_launch_application_cb(GtkAction *action, gpointer data) {
  EomImage *image;
  GAppInfo *app;
  GFile *file;
  GList *files = NULL;
...
static void eom_window_update_openwith_menu(EomWindow *window,
                                            EomImage *image) {
  ...
  for (iter = apps; iter; iter = iter->next) {
    ...
    if (g_ascii_strcasecmp(g_app_info_get_executable(app), g_get_prgname()) ==
        0) {
      g_object_unref(app);
      continue;
    }
    ...
    gtk_ui_manager_add_ui(
        priv->ui_mgr, priv->open_with_menu_id,
        "/MainMenu/Image/ImageOpenWith/Applications Placeholder", name, name,
        GTK_UI_MANAGER_MENUITEM, FALSE);

    gtk_ui_manager_add_ui(
        priv->ui_mgr, priv->open_with_menu_id,
        "/ThumbnailPopup/ImageOpenWith/Applications Placeholder", name, name,
        GTK_UI_MANAGER_MENUITEM, FALSE);
    gtk_ui_manager_add_ui(priv->ui_mgr, priv->open_with_menu_id,
                          "/ViewPopup/ImageOpenWith/Applications Placeholder",
                          name, name, GTK_UI_MANAGER_MENUITEM, FALSE);

    path = g_strdup_printf(
        "/MainMenu/Image/ImageOpenWith/Applications Placeholder/%s", name);
    ...
    path = g_strdup_printf(
        "/ThumbnailPopup/ImageOpenWith/Applications Placeholder/%s", name);
    ...
    path = g_strdup_printf(
        "/ViewPopup/ImageOpenWith/Applications Placeholder/%s", name);
...
static void eom_window_clear_load_job(EomWindow *window) {
  ...
  if (priv->load_job != NULL) {
    if (!priv->load_job->finished) eom_job_queue_remove_job(priv->load_job);

    g_signal_handlers_disconnect_by_func(priv->load_job, eom_job_progress_cb,
                                         window);

    g_signal_handlers_disconnect_by_func(priv->load_job, eom_job_load_cb,
                                         window);
...
static void eom_job_progress_cb(EomJobLoad *job, float progress,
                                gpointer user_data) {
  EomWindow *window;
  ...
  eom_statusbar_set_progress(EOM_STATUSBAR(window->priv->statusbar), progress);
}
...
static void eom_job_save_progress_cb(EomJobSave *job, float progress,
  ...
  eom_statusbar_set_progress(EOM_STATUSBAR(priv->statusbar), progress);
  ...
  if (image != job->current_image) {
    ...
    status_message = g_strdup_printf(_("Saving image \"%s\" (%u/%u)"),
                                     str_image, job->current_pos + 1, n_images);
    g_free(str_image);
  ...
  if (progress == 1.0) image = NULL;
}

static void eom_window_obtain_desired_size(EomImage *image, gint width,
                                           gint height, EomWindow *window) {
  GdkScreen *screen;
  GdkDisplay *display;
  ...
  eom_debug_message(DEBUG_WINDOW, "Setting window size: %d x %d", final_width,
                    final_height);
...
static void eom_window_error_message_area_response(GtkInfoBar *message_area,
                                                   gint response_id,
                                                   EomWindow *window) {
...
static void eom_job_load_cb(EomJobLoad *job, gpointer data) {
  ...
  if (priv->image != NULL) {
    g_signal_handlers_disconnect_by_func(priv->image, image_thumb_changed_cb,
                                         window);
    g_signal_handlers_disconnect_by_func(priv->image, image_file_changed_cb,
                                         window);
  ...
  if (EOM_JOB(job)->error == NULL) {
    ...
    message_area = eom_image_load_error_message_area_new(
        eom_image_get_caption(job->image), EOM_JOB(job)->error);
    ...
    gtk_window_set_title(GTK_WINDOW(window), eom_image_get_caption(job->image));
...
static void eom_window_clear_transform_job(EomWindow *window) {
  EomWindowPrivate *priv = window->priv;
...
static void apply_transformation(EomWindow *window, EomTransform *trans) {
  ...
  g_signal_connect(priv->transform_job, "finished",
                   G_CALLBACK(eom_job_transform_cb), window);

  g_signal_connect(priv->transform_job, "progress",
                   G_CALLBACK(eom_job_progress_cb), window);

  eom_job_queue_add_job(priv->transform_job);
}

static void handle_image_selection_changed_cb(EomThumbView *thumbview,
                                              EomWindow *window) {
  EomWindowPrivate *priv;
  EomImage *image;
  ...
  image =
      eom_thumb_view_get_first_selected_image(EOM_THUMB_VIEW(priv->thumbview));
  ...
  if (priv->status == EOM_WINDOW_STATUS_INIT) {
    g_signal_connect(image, "size-prepared",
                     G_CALLBACK(eom_window_obtain_desired_size), window);
  }

  priv->load_job = eom_job_load_new(image, EOM_IMAGE_DATA_ALL);

  g_signal_connect(priv->load_job, "finished", G_CALLBACK(eom_job_load_cb),
                   window);

  g_signal_connect(priv->load_job, "progress", G_CALLBACK(eom_job_progress_cb),
                   window);
  ...
  status_message = g_strdup_printf(_("Opening image \"%s\""), str_image);
...
static void view_zoom_changed_cb(GtkWidget *widget, double zoom,
                                 gpointer user_data) {
  EomWindow *window;
  ...
  action_zoom_out =
      gtk_action_group_get_action(window->priv->actions_image, "ViewZoomOut");
...
static void eom_window_open_recent_cb(GtkAction *action, EomWindow *window) {
  ...
  eom_application_open_uri_list(EOM_APP, list, GDK_CURRENT_TIME, 0, NULL);
...
static void file_open_dialog_response_cb(GtkWidget *chooser, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    eom_application_open_uri_list(EOM_APP, uris, GDK_CURRENT_TIME, 0, NULL);
...
static void eom_window_update_fullscreen_action(EomWindow *window) {
  ...
  g_signal_handlers_block_by_func(action, eom_window_cmd_fullscreen, window);
...
static void eom_window_update_slideshow_action(EomWindow *window) {
  ...
  g_signal_handlers_block_by_func(action, eom_window_cmd_slideshow, window);
...
static void eom_window_update_pause_slideshow_action(EomWindow *window) {
  ...
  g_signal_handlers_block_by_func(action, eom_window_cmd_pause_slideshow,
                                  window);
...
static void eom_window_update_fullscreen_popup(EomWindow *window) {
  ...
  gtk_widget_set_size_request(popup, screen_rect.width, -1);

  gtk_window_move(GTK_WINDOW(popup), screen_rect.x, screen_rect.y);
}

static void screen_size_changed_cb(GdkScreen *screen, EomWindow *window) {
...
static gboolean slideshow_is_loop_end(EomWindow *window) {
  EomWindowPrivate *priv = window->priv;
  EomImage *image = NULL;
  gint pos;
...
static gboolean slideshow_switch_cb(gpointer data) {
  EomWindow *window = EOM_WINDOW(data);
  EomWindowPrivate *priv = window->priv;
...
static void slideshow_clear_timeout(EomWindow *window) {
  eom_debug(DEBUG_WINDOW);
...
static void slideshow_set_timeout(EomWindow *window) {
  GSource *source;
  ...
  if (window->priv->slideshow_switch_timeout <= 0) return;
...
static void show_fullscreen_popup(EomWindow *window) {
  eom_debug(DEBUG_WINDOW);
...
static void exit_fullscreen_button_clicked_cb(GtkWidget *button,
                                              EomWindow *window) {
  GtkAction *action;
...
static GtkWidget *eom_window_get_exit_fullscreen_button(EomWindow *window) {
  ...
  g_signal_connect(button, "clicked",
                   G_CALLBACK(exit_fullscreen_button_clicked_cb), window);

  return button;
}

static GtkWidget *eom_window_create_fullscreen_popup(EomWindow *window) {
  GtkWidget *popup;
  GtkWidget *hbox;
  GtkWidget *button;
  GtkWidget *toolbar;
  ...
  toolbar =
      gtk_ui_manager_get_widget(window->priv->ui_mgr, "/FullscreenToolbar");
  ...
  g_signal_connect_object(screen, "size-changed",
                          G_CALLBACK(screen_size_changed_cb), window, 0);

  g_signal_connect(popup, "enter-notify-event",
                   G_CALLBACK(fullscreen_leave_notify_cb), window);
...
static void update_ui_visibility(EomWindow *window) {
  EomWindowPrivate *priv;
...
static void eom_window_inhibit_screensaver(EomWindow *window) {
  EomWindowPrivate *priv = window->priv;
  ...
  window->priv->fullscreen_idle_inhibit_cookie = gtk_application_inhibit(
      GTK_APPLICATION(EOM_APP), GTK_WINDOW(window),
      GTK_APPLICATION_INHIBIT_IDLE, _("Viewing a slideshow"));
}

static void eom_window_uninhibit_screensaver(EomWindow *window) {
  EomWindowPrivate *priv = window->priv;

  if (G_UNLIKELY(priv->fullscreen_idle_inhibit_cookie == 0)) return;
...
static void eom_window_run_fullscreen(EomWindow *window, gboolean slideshow) {
  ...
  if (window->priv->fullscreen_popup == NULL)
    priv->fullscreen_popup = eom_window_create_fullscreen_popup(window);
  ...
  g_signal_connect(priv->view, "motion-notify-event",
                   G_CALLBACK(fullscreen_motion_notify_cb), window);
  ...
  if (slideshow) {
    priv->slideshow_random = g_settings_get_boolean(priv->fullscreen_settings,
                                                    EOM_CONF_FULLSCREEN_RANDOM);

    priv->slideshow_loop = g_settings_get_boolean(priv->fullscreen_settings,
                                                  EOM_CONF_FULLSCREEN_LOOP);

    priv->slideshow_switch_timeout = g_settings_get_int(
        priv->fullscreen_settings, EOM_CONF_FULLSCREEN_SECONDS);
  ...
  eom_scroll_view_set_zoom_upscale(EOM_SCROLL_VIEW(priv->view), upscale);
...
static void eom_window_stop_fullscreen(EomWindow *window, gboolean slideshow) {
  ...
  g_signal_handlers_disconnect_by_func(priv->view, fullscreen_motion_notify_cb,
                                       window);

  g_signal_handlers_disconnect_by_func(priv->view, fullscreen_leave_notify_cb,
                                       window);

  g_signal_handlers_disconnect_by_func(priv->thumbview,
                                       fullscreen_motion_notify_cb, window);

  g_signal_handlers_disconnect_by_func(priv->thumbview,
                                       fullscreen_leave_notify_cb, window);
  ...
  eom_scroll_view_override_bg_color(EOM_SCROLL_VIEW(window->priv->view), NULL);
...
static void eom_window_print(EomWindow *window) {
  GtkWidget *dialog;
  GError *error = NULL;
  ...
  print =
      eom_print_operation_new(window->priv->image, print_settings, page_setup);

  // Disable page setup options if they are locked down
  page_setup_disabled = g_settings_get_boolean(
      window->priv->lockdown_settings, EOM_CONF_LOCKDOWN_CAN_SETUP_PAGE);
...
static void eom_window_cmd_file_open(GtkAction *action, gpointer user_data) {
  EomWindow *window;
  EomWindowPrivate *priv;
  EomImage *current;
  GtkWidget *dlg;
  ...
  if (current != NULL) {
    ...
    gtk_file_chooser_set_current_folder_uri(GTK_FILE_CHOOSER(dlg), dir_uri);
    g_free(file_uri);
    g_free(dir_uri);
    ...
    if (use_fallback && pics_dir) {
      gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(dlg), pics_dir);
    }
  }

  g_signal_connect(dlg, "response", G_CALLBACK(file_open_dialog_response_cb),
                   window);

  gtk_widget_show_all(dlg);
}

static void eom_job_close_save_cb(EomJobSave *job, gpointer user_data) {
  EomWindow *window = EOM_WINDOW(user_data);

  g_signal_handlers_disconnect_by_func(job, eom_job_close_save_cb, window);

  gtk_widget_destroy(GTK_WIDGET(window));
}

static void close_confirmation_dialog_response_handler(
    EomCloseConfirmationDialog *dlg, gint response_id, EomWindow *window) {
    ...
    case GTK_RESPONSE_YES:
      ...
      if (eom_window_save_images(window, selected_images)) {
        g_signal_connect(priv->save_job, "finished",
                         G_CALLBACK(eom_job_close_save_cb), window);
...
static gboolean eom_window_unsaved_images_confirm(EomWindow *window) {
  EomWindowPrivate *priv;
  gboolean disabled;
  GtkWidget *dialog;
  GList *list;
  ...
  if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(priv->store), &iter)) {
    do {
      gtk_tree_model_get(GTK_TREE_MODEL(priv->store), &iter,
                         EOM_LIST_STORE_EOM_IMAGE, &image, -1);
      if (!image) continue;
  ...
  if (list) {
    ...
    dialog = eom_close_confirmation_dialog_new(GTK_WINDOW(window), list);
...
static void eom_window_cmd_close_window(GtkAction *action, gpointer user_data) {
  EomWindow *window;
  EomWindowPrivate *priv;
...
static void eom_window_cmd_preferences(GtkAction *action, gpointer user_data) {
  EomWindow *window;
  GtkWidget *pref_dlg;
...
static void eom_window_cmd_edit_toolbar_cb(GtkDialog *dialog, gint response,
                                           gpointer data) {
  EomWindow *window = EOM_WINDOW(data);
  ...
  if (response == EOM_TB_EDITOR_DLG_RESET_RESPONSE) {
    ...
    editor = g_object_get_data(G_OBJECT(dialog), "EggToolbarEditor");
...
static void eom_window_cmd_edit_toolbar(GtkAction *action,
                                        gpointer *user_data) {
  EomWindow *window;
  GtkWidget *dialog;
  GtkWidget *editor;
  ...
  dialog = gtk_dialog_new_with_buttons(
      _("Toolbar Editor"), GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT,
      _("_Reset to Default"), EOM_TB_EDITOR_DLG_RESET_RESPONSE, "gtk-close",
      GTK_RESPONSE_CLOSE, "gtk-help", GTK_RESPONSE_HELP, NULL);
  ...
  gtk_container_add(
      GTK_CONTAINER(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), editor);
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(eom_window_cmd_edit_toolbar_cb), window);

  gtk_widget_show_all(dialog);
}

static void eom_window_cmd_help(GtkAction *action, gpointer user_data) {
  EomWindow *window;
...
static void eom_window_cmd_about(GtkAction *action, gpointer user_data) {
  EomWindow *window;
  ...
  bytes = g_resources_lookup_data("/org/mate/eom/ui/eom.about",
  ...
  gtk_show_about_dialog(
      GTK_WINDOW(window), "program-name", _("Eye of MATE"), "title",
      _("About Eye of MATE"), "version", VERSION, "copyright",
      _("Copyright \xc2\xa9 2000-2010 Free Software Foundation, Inc.\n"
        "Copyright \xc2\xa9 2011 Perberos\n"
        "Copyright \xc2\xa9 2012-2021 MATE developers"),
      "comments",
      _("Eye of MATE is a simple graphics viewer for the MATE Desktop "
        "Environment."),
      "authors", authors, "documenters", documenters, "translator-credits",
      _("translator-credits"), "website", PACKAGE_URL, "logo-icon-name", "eom",
      "wrap-license", TRUE, "license", license_trans, NULL);
...
static void eom_window_cmd_show_hide_bar(GtkAction *action,
                                         gpointer user_data) {
  EomWindow *window;
  EomWindowPrivate *priv;
  gboolean visible;
...
static void wallpaper_info_bar_response(GtkInfoBar *bar, gint response,
                                        EomWindow *window) {
  if (response == GTK_RESPONSE_YES) {
    GAppInfo *app_info;
    GError *error = NULL;
    ...
    app_info = g_app_info_create_from_commandline(
        ...
        "mate-appearance-properties", G_APP_INFO_CREATE_NONE, &error);
...
static void eom_window_set_wallpaper(EomWindow *window, const gchar *filename,
  ...
  g_settings_set_string(wallpaper_settings, EOM_CONF_BACKGROUND_FILE, filename);
  g_object_unref(wallpaper_settings);

  /* I18N: When setting mnemonics for these strings, watch out to not
     clash with mnemonics from eom's menubar */
  info_bar = gtk_info_bar_new_with_buttons(
      _("_Open Background Preferences"), GTK_RESPONSE_YES,
      C_("MessageArea", "Hi_de"), GTK_RESPONSE_NO, NULL);
  gtk_info_bar_set_message_type(GTK_INFO_BAR(info_bar), GTK_MESSAGE_QUESTION);

  image = gtk_image_new_from_icon_name("dialog-question", GTK_ICON_SIZE_DIALOG);
  label = gtk_label_new(NULL);

  if (!visible_filename) basename = g_path_get_basename(filename);

  /* The newline character is currently necessary due to a problem
   * with the automatic line break. */
  text =
      g_strdup_printf(_("The image \"%s\" has been set as Desktop Background."
  ...
  g_free(markup);
  g_free(text);
  if (!visible_filename) g_free(basename);
  ...
  eom_window_set_message_area(window, info_bar);
  gtk_info_bar_set_default_response(GTK_INFO_BAR(info_bar), GTK_RESPONSE_YES);
  g_signal_connect(info_bar, "response",
                   G_CALLBACK(wallpaper_info_bar_response), window);
}

static void eom_job_save_cb(EomJobSave *job, gpointer user_data) {
  EomWindow *window = EOM_WINDOW(user_data);
  GtkAction *action_save;

  g_signal_handlers_disconnect_by_func(job, eom_job_save_cb, window);

  g_signal_handlers_disconnect_by_func(job, eom_job_save_progress_cb, window);
...
static void eom_job_copy_cb(EomJobCopy *job, gpointer user_data) {
  ...
  /* Move the file */
  g_file_move(source_file, dest_file, G_FILE_COPY_OVERWRITE, NULL, NULL, NULL,
              NULL);
...
static gboolean eom_window_save_images(EomWindow *window, GList *images) {
  ...
  if (window->priv->save_job != NULL) return FALSE;

  priv->save_job = eom_job_save_new(images);

  g_signal_connect(priv->save_job, "finished", G_CALLBACK(eom_job_save_cb),
                   window);

  g_signal_connect(priv->save_job, "progress",
                   G_CALLBACK(eom_job_save_progress_cb), window);

  return TRUE;
}

static void eom_window_cmd_save(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
  EomWindow *window;
  GList *images;
  ...
  if (window->priv->save_job != NULL) return;
...
static GFile *eom_window_retrieve_save_as_file(EomWindow *window,
                                               EomImage *image) {
  GtkWidget *dialog;
...
static void eom_window_cmd_save_as(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
  EomWindow *window;
  GList *images;
  guint n_images;
  ...
  if (n_images == 1) {
    ...
    dialog = eom_save_as_dialog_new(GTK_WINDOW(window), images, base_file);
...
static void eom_window_cmd_open_containing_folder(GtkAction *action,
                                                  gpointer user_data) {
  EomWindowPrivate *priv;
  ...
  eom_util_show_file_in_filemanager(file, GTK_WINDOW(user_data));
}

static void eom_window_cmd_print(GtkAction *action, gpointer user_data) {
  EomWindow *window = EOM_WINDOW(user_data);
 ...
 * @window: a #EomWindow
 *
 * Gets the @window property dialog. The widget will be built on the first call
 * to this function.
 *
...
GtkWidget *eom_window_get_properties_dialog(EomWindow *window) {
  ...
  if (priv->properties_dlg == NULL) {
    ...
    eom_properties_dialog_update(EOM_PROPERTIES_DIALOG(priv->properties_dlg),
                                 priv->image);
    g_settings_bind(priv->ui_settings, EOM_CONF_UI_PROPSDIALOG_NETBOOK_MODE,
                    priv->properties_dlg, "netbook-mode", G_SETTINGS_BIND_GET);
  }

  return priv->properties_dlg;
}

static void eom_window_cmd_properties(GtkAction *action, gpointer user_data) {
  EomWindow *window = EOM_WINDOW(user_data);
  GtkWidget *dialog;
...
static void eom_window_cmd_undo(GtkAction *action, gpointer user_data) {
  g_return_if_fail(EOM_IS_WINDOW(user_data));

  apply_transformation(EOM_WINDOW(user_data), NULL);
}

static void eom_window_cmd_flip_horizontal(GtkAction *action,
                                           gpointer user_data) {
  g_return_if_fail(EOM_IS_WINDOW(user_data));
...
static void eom_window_cmd_flip_vertical(GtkAction *action,
                                         gpointer user_data) {
  g_return_if_fail(EOM_IS_WINDOW(user_data));
...
static void eom_window_cmd_rotate_90(GtkAction *action, gpointer user_data) {
  g_return_if_fail(EOM_IS_WINDOW(user_data));

  apply_transformation(EOM_WINDOW(user_data), eom_transform_rotate_new(90));
}

static void eom_window_cmd_rotate_270(GtkAction *action, gpointer user_data) {
  g_return_if_fail(EOM_IS_WINDOW(user_data));

  apply_transformation(EOM_WINDOW(user_data), eom_transform_rotate_new(270));
}

static void eom_window_cmd_wallpaper(GtkAction *action, gpointer user_data) {
  EomWindow *window;
  EomWindowPrivate *priv;
  EomImage *image;
  GFile *file;
  ...
  if (filename == NULL || !eom_util_file_is_persistent(file)) {
    ...
    gtk_statusbar_push(GTK_STATUSBAR(priv->statusbar), priv->copy_file_cid,
    ...
    g_signal_connect(priv->copy_job, "finished", G_CALLBACK(eom_job_copy_cb),
                     window);
...
static gboolean eom_window_all_images_trasheable(GList *images) {
  GFile *file;
  GFileInfo *file_info;
  GList *iter;
  EomImage *image;
  ...
  for (iter = images; iter != NULL; iter = g_list_next(iter)) {
    ...
    file = eom_image_get_file(image);
    file_info = g_file_query_info(file, G_FILE_ATTRIBUTE_ACCESS_CAN_TRASH, 0,
                                  NULL, NULL);
    can_trash = g_file_info_get_attribute_boolean(
        file_info, G_FILE_ATTRIBUTE_ACCESS_CAN_TRASH);

    g_object_unref(file_info);
    g_object_unref(file);

    if (can_trash == FALSE) break;
  }

  return can_trash;
}

static int show_move_to_trash_confirm_dialog(EomWindow *window, GList *images,
                                             gboolean can_trash) {
  GtkWidget *dlg;
  char *prompt;
  int response;
  ...
  if (can_trash && (dontaskagain || neverask)) return GTK_RESPONSE_OK;
  ...
  if (n_images == 1) {
    ...
    if (can_trash) {
      ...
      prompt = g_strdup_printf(
          _("A trash for \"%s\" couldn't be found. Do you want to remove "
    ...
    if (can_trash) {
      ...
      prompt =
          g_strdup(_("Some of the selected images can't be moved to the trash "
  ...
  dlg = gtk_message_dialog_new_with_markup(
      ...
      "<span weight=\"bold\" size=\"larger\">%s</span>", prompt);
  g_free(prompt);

  gtk_dialog_add_button(GTK_DIALOG(dlg), "gtk-cancel", GTK_RESPONSE_CANCEL);

  if (can_trash) {
    gtk_dialog_add_button(GTK_DIALOG(dlg), _("Move to _Trash"),
                          GTK_RESPONSE_OK);

    dontask_cbutton = gtk_check_button_new_with_mnemonic(
        _("_Do not ask again during this session"));
  ...
  if (can_trash && response == GTK_RESPONSE_OK)
    dontaskagain =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(dontask_cbutton));
...
static gboolean move_to_trash_real(EomImage *image, GError **error) {
  GFile *file;
  GFileInfo *file_info;
  ...
  file = eom_image_get_file(image);
  file_info =
      g_file_query_info(file, G_FILE_ATTRIBUTE_ACCESS_CAN_TRASH, 0, NULL, NULL);
  if (file_info == NULL) {
    g_set_error(error, EOM_WINDOW_ERROR, EOM_WINDOW_ERROR_TRASH_NOT_FOUND,
  ...
  if (can_trash) {
    ...
    if (result == FALSE) {
      g_set_error(error, EOM_WINDOW_ERROR, EOM_WINDOW_ERROR_IO,
...
static void eom_window_cmd_copy_image(GtkAction *action, gpointer user_data) {
...
static void eom_window_cmd_move_to_trash(GtkAction *action,
                                         gpointer user_data) {
  GList *images;
  GList *it;
  EomWindowPrivate *priv;
  ...
  if (pos >= 0) {
    ...
    eom_thumb_view_set_current_image(EOM_THUMB_VIEW(priv->thumbview), img,
                                     TRUE);
...
static void eom_window_cmd_pause_slideshow(GtkAction *action,
  ...
  if (!slideshow && window->priv->mode != EOM_WINDOW_MODE_FULLSCREEN) return;

  eom_window_run_fullscreen(window, !slideshow);
}

static void eom_window_cmd_zoom_in(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
...
static void eom_window_cmd_zoom_out(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
...
static void eom_window_cmd_zoom_normal(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
...
static void eom_window_cmd_zoom_fit(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
...
static void eom_window_cmd_go_prev(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
...
static void eom_window_cmd_go_next(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
...
static void eom_window_cmd_go_first(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
...
static void eom_window_cmd_go_last(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
...
static void eom_window_cmd_go_random(GtkAction *action, gpointer user_data) {
  EomWindowPrivate *priv;
...
static const GtkActionEntry action_entries_window[] = {
    ...
    {"ImageOpen", "document-open", N_("_Open…"), "<control>O",
     N_("Open a file"), G_CALLBACK(eom_window_cmd_file_open)},
    {"ImageClose", "window-close", N_("_Close"), "<control>W",
     N_("Close window"), G_CALLBACK(eom_window_cmd_close_window)},
    ...
    {"EditPreferences", "preferences-desktop", N_("Prefere_nces"), NULL,
     N_("Preferences for Eye of MATE"), G_CALLBACK(eom_window_cmd_preferences)},
    {"HelpManual", "help-browser", N_("_Contents"), "F1",
     N_("Help on this application"), G_CALLBACK(eom_window_cmd_help)},
    {"HelpAbout", "help-about", N_("_About"), NULL,
     N_("About this application"), G_CALLBACK(eom_window_cmd_about)}};
...
static const GtkToggleActionEntry toggle_entries_window[] = {
    ...
    {"ViewStatusbar", NULL, N_("_Statusbar"), NULL,
     ...
     G_CALLBACK(eom_window_cmd_show_hide_bar), TRUE},
    {"ViewImageCollection", "eom-image-collection", N_("_Image Collection"),
     "<control>F9",
     N_("Changes the visibility of the image collection pane in the current "
        "window"),
     G_CALLBACK(eom_window_cmd_show_hide_bar), TRUE},
...
static const GtkActionEntry action_entries_image[] = {
    ...
    {"ImageOpenWith", NULL, N_("Open _with"), NULL,
     N_("Open the selected image with a different application"), NULL},
    ...
    {"ImagePrint", "document-print", N_("_Print…"), "<control>p",
     N_("Print the selected image"), G_CALLBACK(eom_window_cmd_print)},
    ...
    {"EditUndo", "edit-undo", N_("_Undo"), "<control>z",
     N_("Undo the last change in the image"), G_CALLBACK(eom_window_cmd_undo)},
    {"EditFlipHorizontal", "object-flip-horizontal", N_("Flip _Horizontal"),
     NULL, N_("Mirror the image horizontally"),
    ...
    {"EditRotate90", "object-rotate-right", N_("_Rotate Clockwise"),
     "<control>r", N_("Rotate the image 90 degrees to the right"),
     G_CALLBACK(eom_window_cmd_rotate_90)},
    {"EditRotate270", "object-rotate-left", N_("Rotate Counterc_lockwise"),
     "<ctrl><shift>r", N_("Rotate the image 90 degrees to the left"),
    ...
    {"ViewZoomIn", "zoom-in", N_("_Zoom In"), "<control>plus",
     N_("Enlarge the image"), G_CALLBACK(eom_window_cmd_zoom_in)},
    {"ViewZoomOut", "zoom-out", N_("Zoom _Out"), "<control>minus",
     N_("Shrink the image"), G_CALLBACK(eom_window_cmd_zoom_out)},
    ...
    {"ViewZoomFit", "zoom-fit-best", N_("_Best Fit"), "F",
     N_("Fit the image to the window"), G_CALLBACK(eom_window_cmd_zoom_fit)},
    {"ControlEqual", "zoom-in", N_("_Zoom In"), "<control>equal",
     N_("Enlarge the image"), G_CALLBACK(eom_window_cmd_zoom_in)},
    {"ControlKpAdd", "zoom-in", N_("_Zoom In"), "<control>KP_Add",
     N_("Shrink the image"), G_CALLBACK(eom_window_cmd_zoom_in)},
    {"ControlKpSub", "zoom-out", N_("Zoom _Out"), "<control>KP_Subtract",
     N_("Shrink the image"), G_CALLBACK(eom_window_cmd_zoom_out)},
    {"Delete", NULL, N_("Move to _Trash"), "Delete", NULL,
     G_CALLBACK(eom_window_cmd_move_to_trash)},
};
...
static const GtkToggleActionEntry toggle_entries_image[] = {
    ...
    {"PauseSlideshow", "media-playback-pause", N_("Pause Slideshow"), NULL,
     N_("Pause or resume the slideshow"),
...
static const GtkActionEntry action_entries_collection[] = {
    ...
    {"BackSpace", NULL, N_("_Previous Image"), "BackSpace", NULL,
     G_CALLBACK(eom_window_cmd_go_prev)},
    {"Home", NULL, N_("_First Image"), "Home", NULL,
     G_CALLBACK(eom_window_cmd_go_first)},
    {"End", NULL, N_("_Last Image"), "End", NULL,
     G_CALLBACK(eom_window_cmd_go_last)},
};
...
static void menu_item_select_cb(GtkMenuItem *proxy, EomWindow *window) {
  GtkAction *action;
  char *message;
...
static void menu_item_deselect_cb(GtkMenuItem *proxy, EomWindow *window) {
  gtk_statusbar_pop(GTK_STATUSBAR(window->priv->statusbar),
...
static void disconnect_proxy_cb(GtkUIManager *manager, GtkAction *action,
                                GtkWidget *proxy, EomWindow *window) {
  if (GTK_IS_MENU_ITEM(proxy)) {
    g_signal_handlers_disconnect_by_func(proxy, menu_item_select_cb, window);
    g_signal_handlers_disconnect_by_func(proxy, menu_item_deselect_cb, window);
  }
}

static void set_action_properties(GtkActionGroup *window_group,
  ...
  action =
      gtk_action_group_get_action(image_group, "ImageOpenContainingFolder");
...
static gint sort_recents_mru(GtkRecentInfo *a, GtkRecentInfo *b) {
  ...
  has_eom_a = gtk_recent_info_has_application(a, EOM_RECENT_FILES_APP_NAME);
  has_eom_b = gtk_recent_info_has_application(b, EOM_RECENT_FILES_APP_NAME);
  if (has_eom_a && has_eom_b) {
    ...
    gtk_recent_info_get_application_info(a, EOM_RECENT_FILES_APP_NAME, NULL,
                                         NULL, &time_a);
    gtk_recent_info_get_application_info(b, EOM_RECENT_FILES_APP_NAME, NULL,
                                         NULL, &time_b);
...
static void eom_window_update_recent_files_menu(EomWindow *window) {
  EomWindowPrivate *priv;
  ...
  for (li = actions; li != NULL; li = li->next) {
    g_signal_handlers_disconnect_by_func(li->data, eom_window_open_recent_cb,
                                         window);
  ...
  for (li = items; li != NULL && count_recent < EOM_RECENT_FILES_LIMIT;
       li = li->next) {
    gchar *action_name;
    gchar *label;
    gchar *tip;
    ...
    label = g_strdup_printf("%s_%d. %s", (is_rtl ? "\xE2\x80\x8F" : ""),
                            count_recent, label_filename);
    ...
    gtk_ui_manager_add_ui(priv->ui_mgr, priv->recent_menu_id,
                          "/MainMenu/Image/RecentDocuments", action_name,
...
static void eom_window_recent_manager_changed_cb(GtkRecentManager *manager,
                                                 EomWindow *window) {
  eom_window_update_recent_files_menu(window);
}

static void eom_window_drag_data_received(GtkWidget *widget,
  ...
  if (!gtk_targets_include_uri(&target, 1)) return;
  ...
  if (gdk_drag_context_get_suggested_action(context) == GDK_ACTION_COPY) {
    ...
    file_list = eom_util_parse_uri_string_list_to_file_list(
...
static void eom_window_set_drag_dest(EomWindow *window) {
  gtk_drag_dest_set(GTK_WIDGET(window),
                    GTK_DEST_DEFAULT_MOTION | GTK_DEST_DEFAULT_DROP, NULL, 0,
                    GDK_ACTION_COPY | GDK_ACTION_ASK);
  gtk_drag_dest_add_uri_targets(GTK_WIDGET(window));
}

static void eom_window_sidebar_visibility_changed(GtkWidget *widget,
                                                  EomWindow *window) {
  GtkAction *action;
  gboolean visible;
  ...
  action =
      gtk_action_group_get_action(window->priv->actions_window, "ViewSidebar");
...
static void eom_window_sidebar_page_added(EomSidebar *sidebar,
...
static void eom_window_sidebar_page_removed(EomSidebar *sidebar,
                                            GtkWidget *main_widget,
                                            EomWindow *window) {
...
static void eom_window_open_editor(GtkAction *action, EomWindow *window) {
  GdkAppLaunchContext *context;
  GAppInfo *app_info;
  GList *files = NULL;
  GFile *file;
  ...
  g_app_info_launch(app_info, files, G_APP_LAUNCH_CONTEXT(context), NULL);
...
static void eom_window_add_open_editor_action(EomWindow *window) {
  EggToolbarsModel *model;
  GAppInfo *app_info;
  GtkAction *action;
  gchar *tooltip;
  ...
  g_signal_connect(action, "activate", G_CALLBACK(eom_window_open_editor),
                   window);
...
static void eom_window_construct_ui(EomWindow *window) {
  EomWindowPrivate *priv;
  ...
  gtk_action_group_add_actions(priv->actions_window, action_entries_window,
                               G_N_ELEMENTS(action_entries_window), window);
  ...
  gtk_action_group_add_actions(priv->actions_image, action_entries_image,
                               G_N_ELEMENTS(action_entries_image), window);
  ...
  gtk_action_group_add_actions(priv->actions_collection,
                               ...
                               G_N_ELEMENTS(action_entries_collection), window);
  ...
  if (!gtk_ui_manager_add_ui_from_resource(
          priv->ui_mgr, "/org/mate/eom/ui/eom-ui.xml", &error)) {
    g_warning("building menus failed: %s", error->message);
    g_error_free(error);
  }

  g_signal_connect(priv->ui_mgr, "connect_proxy", G_CALLBACK(connect_proxy_cb),
                   window);
  ...
  menuitem =
      gtk_ui_manager_get_widget(priv->ui_mgr, "/MainMenu/Edit/EditRotate90");
  gtk_image_menu_item_set_always_show_image(GTK_IMAGE_MENU_ITEM(menuitem),
                                            TRUE);

  menuitem =
      gtk_ui_manager_get_widget(priv->ui_mgr, "/MainMenu/Edit/EditRotate270");
  gtk_image_menu_item_set_always_show_image(GTK_IMAGE_MENU_ITEM(menuitem),
                                            TRUE);

  priv->toolbar = GTK_WIDGET(
      g_object_new(EGG_TYPE_EDITABLE_TOOLBAR, "ui-manager", priv->ui_mgr,
                   "popup-path", "/ToolbarPopup", "model",
                   eom_application_get_toolbars_model(EOM_APP), NULL));

  gtk_style_context_add_class(
      gtk_widget_get_style_context(GTK_WIDGET(priv->toolbar)),
      GTK_STYLE_CLASS_PRIMARY_TOOLBAR);

  egg_editable_toolbar_show(EGG_EDITABLE_TOOLBAR(priv->toolbar), "Toolbar");
  ...
  gtk_box_pack_end(GTK_BOX(priv->box), GTK_WIDGET(priv->statusbar), FALSE,
                   FALSE, 0);
  gtk_widget_show(priv->statusbar);

  priv->image_info_message_cid = gtk_statusbar_get_context_id(
      GTK_STATUSBAR(priv->statusbar), "image_info_message");
  ...
  g_signal_connect_after(priv->sidebar, "page-added",
                         G_CALLBACK(eom_window_sidebar_page_added), window);

  g_signal_connect_after(priv->sidebar, "page-removed",
                         G_CALLBACK(eom_window_sidebar_page_removed), window);

  priv->view = eom_scroll_view_new();

  eom_sidebar_add_page(EOM_SIDEBAR(priv->sidebar), _("Properties"),
                       GTK_WIDGET(eom_metadata_sidebar_new(window)));

  gtk_widget_set_size_request(GTK_WIDGET(priv->view), 100, 100);
  g_signal_connect(priv->view, "zoom_changed", G_CALLBACK(view_zoom_changed_cb),
                   window);
  ...
  eom_scroll_view_set_popup(EOM_SCROLL_VIEW(priv->view), GTK_MENU(view_popup));

  gtk_paned_pack1(GTK_PANED(hpaned), priv->sidebar, FALSE, FALSE);

  gtk_paned_pack2(GTK_PANED(hpaned), priv->view, TRUE, FALSE);
  ...
  priv->nav = eom_thumb_nav_new(
      priv->thumbview, EOM_THUMB_NAV_MODE_ONE_ROW,
      g_settings_get_boolean(priv->ui_settings, EOM_CONF_UI_SCROLL_BUTTONS));

  // Bind the scroll buttons to their GSettings key
  g_settings_bind(priv->ui_settings, EOM_CONF_UI_SCROLL_BUTTONS, priv->nav,
                  "show-buttons", G_SETTINGS_BIND_GET);
  ...
  eom_window_can_save_changed_cb(priv->lockdown_settings,
                                 EOM_CONF_LOCKDOWN_CAN_SAVE, window);
...
static void eom_window_init(EomWindow *window) {
  GdkGeometry hints;
#if defined(HAVE_LCMS) && defined(GDK_WINDOWING_X11)
  GdkScreen *screen;
#endif
  ...
  g_signal_connect(priv->lockdown_settings,
                   ...
                   G_CALLBACK(eom_window_can_save_changed_cb), window);
  ...
  gtk_window_set_geometry_hints(GTK_WINDOW(window), GTK_WIDGET(window), &hints,
                                GDK_HINT_MIN_SIZE);

  gtk_window_set_default_size(GTK_WINDOW(window), EOM_WINDOW_DEFAULT_WIDTH,
...
#if defined(HAVE_LCMS) && defined(GDK_WINDOWING_X11)
  screen = gtk_widget_get_screen(GTK_WIDGET(window));
  window->priv->display_profile = eom_window_get_display_profile(screen);
#endif
...
static void eom_window_dispose(GObject *object) {
  EomWindow *window;
  EomWindowPrivate *priv;
  ...
  if (priv->thumbview) {
    ...
    g_signal_handlers_disconnect_by_func(
        priv->thumbview, handle_image_selection_changed_cb, window);
    g_clear_object(&priv->thumbview);
  }

  if (priv->store != NULL) {
    g_signal_handlers_disconnect_by_func(
        priv->store, eom_window_list_store_image_added, window);
    g_signal_handlers_disconnect_by_func(
        priv->store, eom_window_list_store_image_removed, window);
...
static gint eom_window_delete(GtkWidget *widget, GdkEventAny *event) {
  EomWindow *window;
  EomWindowPrivate *priv;
...
static gint eom_window_key_press(GtkWidget *widget, GdkEventKey *event) {
  GtkContainer *tbcontainer =
      GTK_CONTAINER((EOM_WINDOW(widget)->priv->toolbar));
  gint result = FALSE;
  ...
  switch (event->keyval) {
    ...
    case GDK_KEY_P:
      if (EOM_WINDOW(widget)->priv->mode == EOM_WINDOW_MODE_FULLSCREEN ||
          EOM_WINDOW(widget)->priv->mode == EOM_WINDOW_MODE_SLIDESHOW) {
        gboolean slideshow;
    ...
    case GDK_KEY_Up:
      if (eom_scroll_view_scrollbars_visible(
              EOM_SCROLL_VIEW(EOM_WINDOW(widget)->priv->view))) {
        /* break to let scrollview handle the key */
        break;
      }
      if (gtk_container_get_focus_child(tbcontainer) != NULL) break;
  ...
  if (result == FALSE &&
      GTK_WIDGET_CLASS(eom_window_parent_class)->key_press_event) {
    result = (*GTK_WIDGET_CLASS(eom_window_parent_class)->key_press_event)(
        widget, event);
  }
...
static gint eom_window_button_press(GtkWidget *widget, GdkEventButton *event) {
  ...
  if (result == FALSE &&
      GTK_WIDGET_CLASS(eom_window_parent_class)->button_press_event) {
    result = (*GTK_WIDGET_CLASS(eom_window_parent_class)->button_press_event)(
        widget, event);
  }
...
static gboolean eom_window_focus_out_event(GtkWidget *widget,
                                           GdkEventFocus *event) {
  EomWindow *window = EOM_WINDOW(widget);
  EomWindowPrivate *priv = window->priv;
  gboolean fullscreen;
  ...
  return GTK_WIDGET_CLASS(eom_window_parent_class)
      ->focus_out_event(widget, event);
}

static void eom_window_set_property(GObject *object, guint property_id,
...
static void eom_window_get_property(GObject *object, guint property_id,
...
static void on_extension_added(PeasExtensionSet *set, PeasPluginInfo *info,
                               PeasExtension *exten, GtkWindow *window) {
...
static GObject *eom_window_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_params) {
  GObject *object;
  EomWindowPrivate *priv;

  object = G_OBJECT_CLASS(eom_window_parent_class)
  ...
  priv->extensions = peas_extension_set_new(
      PEAS_ENGINE(EOM_APP->priv->plugin_engine), EOM_TYPE_WINDOW_ACTIVATABLE,
      "window", EOM_WINDOW(object), NULL);
...
static void eom_window_class_init(EomWindowClass *class) {
   ...
   */
  g_object_class_install_property(
      g_object_class, PROP_STARTUP_FLAGS,
      g_param_spec_flags("startup-flags", NULL, NULL, EOM_TYPE_STARTUP_FLAGS, 0,
  ...
  signals[SIGNAL_PREPARED] =
      g_signal_new("prepared", EOM_TYPE_WINDOW, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EomWindowClass, prepared), NULL, NULL,
...
GtkWidget *eom_window_new(EomStartupFlags flags) {
  ...
  window = EOM_WINDOW(g_object_new(EOM_TYPE_WINDOW, "type", GTK_WINDOW_TOPLEVEL,
                                   "application", EOM_APP, "show-menubar",
                                   FALSE, "startup-flags", flags, NULL));
...
static void eom_job_model_cb(EomJobModel *job, gpointer data) {
  EomWindow *window;
  EomWindowPrivate *priv;
  gint n_images;
  ...
  g_signal_connect(priv->store, "row-inserted",
                   G_CALLBACK(eom_window_list_store_image_added), window);

  g_signal_connect(priv->store, "row-deleted",
                   G_CALLBACK(eom_window_list_store_image_removed), window);
...
void eom_window_open_file_list(EomWindow *window, GSList *file_list) {
  ...
  g_signal_connect(job, "finished", G_CALLBACK(eom_job_model_cb), window);
...
void eom_window_set_mode(EomWindow *window, EomWindowMode mode) {
  ...
  if (window->priv->mode == mode) return;

  switch (mode) {
    case EOM_WINDOW_MODE_NORMAL:
      eom_window_stop_fullscreen(
          window, window->priv->mode == EOM_WINDOW_MODE_SLIDESHOW);
      break;
