...
static void eom_image_free_mem_private(EomImage *image) {
...
static void eom_image_dispose(GObject *object) {
  EomImagePrivate *priv;
...
static void eom_image_class_init(EomImageClass *klass) {
  GObjectClass *object_class = (GObjectClass *)klass;

  object_class->dispose = eom_image_dispose;

  signals[SIGNAL_SIZE_PREPARED] = g_signal_new(
      "size-prepared", EOM_TYPE_IMAGE, G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EomImageClass, size_prepared), NULL, NULL,
      eom_marshal_VOID__INT_INT, G_TYPE_NONE, 2, G_TYPE_INT, G_TYPE_INT);

  signals[SIGNAL_CHANGED] =
      g_signal_new("changed", EOM_TYPE_IMAGE, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EomImageClass, changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[SIGNAL_THUMBNAIL_CHANGED] =
      g_signal_new("thumbnail-changed", EOM_TYPE_IMAGE, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EomImageClass, thumbnail_changed), NULL,
                   NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[SIGNAL_SAVE_PROGRESS] = g_signal_new(
      "save-progress", EOM_TYPE_IMAGE, G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EomImageClass, save_progress), NULL, NULL,
  ...
  signals[SIGNAL_NEXT_FRAME] =
      g_signal_new("next-frame", EOM_TYPE_IMAGE, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EomImageClass, next_frame), NULL, NULL,
                   g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);

  signals[SIGNAL_FILE_CHANGED] =
      g_signal_new("file-changed", EOM_TYPE_IMAGE, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EomImageClass, file_changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void eom_image_init(EomImage *img) {
...
static void eom_image_update_exif_data(EomImage *image) {
#ifdef HAVE_EXIF
  EomImagePrivate *priv;
  ExifEntry *entry;
  ExifByteOrder bo;
...
static void eom_image_real_transform(EomImage *img, EomTransform *trans,
                                     gboolean is_undo, EomJob *job) {
  EomImagePrivate *priv;
...
static gboolean do_emit_size_prepared_signal(EomImage *img) {
  g_signal_emit(img, signals[SIGNAL_SIZE_PREPARED], 0, img->priv->width,
                img->priv->height);
  return FALSE;
}

static void eom_image_emit_size_prepared(EomImage *img) {
  g_idle_add_full(G_PRIORITY_DEFAULT_IDLE,
                  (GSourceFunc)do_emit_size_prepared_signal, g_object_ref(img),
                  g_object_unref);
}

static void eom_image_size_prepared(GdkPixbufLoader *loader, gint width,
...
static EomMetadataReader *check_for_metadata_img_format(EomImage *img,
                                                        guchar *buffer,
                                                        guint bytes_read) {
  EomMetadataReader *md_reader = NULL;

  eom_debug_message(DEBUG_IMAGE_DATA,
                    "Check image format for jpeg: %x%x - length: %i", buffer[0],
                    buffer[1], bytes_read);
  ...
  if (bytes_read >= 2) {
    ...
    if (bytes_read >= 8 && memcmp(buffer, "\x89PNG\x0D\x0A\x1a\x0A", 8) == 0) {
...
static gboolean eom_image_needs_transformation(EomImage *img) {
  g_return_val_if_fail(EOM_IS_IMAGE(img), FALSE);

  return (img->priv->trans != NULL || img->priv->trans_autorotate != NULL);
}

static gboolean eom_image_apply_transformations(EomImage *img, GError **error) {
  ...
  if (priv->image == NULL) {
    g_set_error(error, EOM_IMAGE_ERROR, EOM_IMAGE_ERROR_NOT_LOADED,
  ...
  if (priv->trans != NULL && priv->trans_autorotate != NULL) {
    composition = eom_transform_compose(priv->trans, priv->trans_autorotate);
  ...
  if (transformed != NULL) {
    ...
    g_set_error(error, EOM_IMAGE_ERROR, EOM_IMAGE_ERROR_GENERIC,
...
static void eom_image_get_file_info(EomImage *img, goffset *bytes,
  ...
  if (file_info == NULL) {
    ...
    g_set_error(error, EOM_IMAGE_ERROR, EOM_IMAGE_ERROR_VFS,
                "Error in getting image file info");
  } else {
    if (bytes) *bytes = g_file_info_get_size(file_info);
...
#if defined(HAVE_LCMS) && defined(GDK_WINDOWING_X11)
void eom_image_apply_display_profile(EomImage *img, cmsHPROFILE screen) {
  EomImagePrivate *priv;
  ...
  if (priv->profile == NULL) {
    ...
    const char *data = gdk_pixbuf_get_option(priv->image, "icc-profile");

    if (data) {
      gsize profile_size = 0;
      guchar *profile_data = g_base64_decode(data, &profile_size);

      if (profile_data && profile_size > 0) {
        eom_debug_message(DEBUG_LCMS,
                          "Using ICC profile "
                          "extracted by GdkPixbuf");
        priv->profile = cmsOpenProfileFromMem(profile_data, profile_size);
        g_free(profile_data);
      }
    }
  ...
  if (gdk_pixbuf_get_has_alpha(priv->image)) color_type = TYPE_RGBA_8;

  transform = cmsCreateTransform(priv->profile, color_type, screen, color_type,
                                 INTENT_PERCEPTUAL, 0);
...
static void eom_image_set_icc_data(EomImage *img,
                                   EomMetadataReader *md_reader) {
...
static void eom_image_set_orientation(EomImage *img) {
  EomImagePrivate *priv;
#ifdef HAVE_EXIF
  ExifData *exif;
#endif
...
#endif
  {
    ...
    if (pbuf) {
      ...
      if (o_str) {
        short t = (short)g_ascii_strtoll(o_str, NULL, 10);
        if (t >= 0 && t < 9) priv->orientation = t;
      }
      g_object_unref(pbuf);
    }
  }

  if (priv->orientation > 4 && priv->orientation < 9) {
    gint tmp;
...
static void eom_image_real_autorotate(EomImage *img) {
  static const EomTransformType lookup[8] = {
      EOM_TRANSFORM_NONE,       EOM_TRANSFORM_FLIP_HORIZONTAL,
      EOM_TRANSFORM_ROT_180,    EOM_TRANSFORM_FLIP_VERTICAL,
      EOM_TRANSFORM_TRANSPOSE,  EOM_TRANSFORM_ROT_90,
      EOM_TRANSFORM_TRANSVERSE, EOM_TRANSFORM_ROT_270};
  EomImagePrivate *priv;
...
#ifdef HAVE_EXEMPI
static void eom_image_set_xmp_data(EomImage *img,
                                   EomMetadataReader *md_reader) {
  EomImagePrivate *priv;
...
static void eom_image_set_exif_data(EomImage *img,
                                    EomMetadataReader *md_reader) {
  EomImagePrivate *priv;
  ...
  eom_metadata_reader_get_exif_chunk(md_reader, &priv->exif_chunk,
                                     &priv->exif_chunk_len);
#endif
}
 ...
 **/
static gboolean eom_image_get_dimension_from_thumbnail(EomImage *image,
                                                       gint *width,
                                                       gint *height) {
  if (image->priv->thumbnail == NULL) return FALSE;
...
static gboolean eom_image_real_load(EomImage *img, guint data2read, EomJob *job,
                                    GError **error) {
  EomImagePrivate *priv;
  ...
  if (read_only_dimension) {
    ...
    done = eom_image_get_dimension_from_thumbnail(img, &width, &height);
  ...
  if (read_image_data || read_only_dimension) {
    ...
    if (!use_rsvg) {
      ...
      g_signal_connect_object(loader, "size-prepared",
                              G_CALLBACK(eom_image_size_prepared), img, 0);
    }
  }
  g_free(mime_type);
  ...
  while (!priv->cancel_loading) {
    ...
    bytes_read = g_input_stream_read(G_INPUT_STREAM(input_stream), buffer,
                                     EOM_IMAGE_READ_BUFFER_SIZE, NULL, error);
    ...
    if ((read_image_data || read_only_dimension)) {
      ...
      if (use_rsvg) {
        ...
        res = rsvg_handle_write(priv->svg, buffer, bytes_read, error);
    ...
    if (md_reader != NULL) {
      ...
      if (eom_metadata_reader_finished(md_reader)) {
        ...
        if (data2read == EOM_IMAGE_DATA_EXIF) break;
      }
    }
  ...
  if (read_image_data || read_only_dimension) {
    ...
    if (use_rsvg) {
      ...
      rsvg_handle_close(priv->svg, (failed ? NULL : error));
    } else
#endif
        if (failed) {
  ...
  failed = (failed || priv->cancel_loading || bytes_read_total == 0 ||
            (error && *error != NULL));
  ...
  if (failed) {
    ...
    if (G_LIKELY(priv->image != NULL)) {
      if (!use_rsvg) g_object_ref(priv->image);
...
gboolean eom_image_has_data(EomImage *img, EomImageData req_data) {
  EomImagePrivate *priv;
...
gboolean eom_image_load(EomImage *img, EomImageData data2read, EomJob *job,
                        GError **error) {
  EomImagePrivate *priv;
...
void eom_image_set_thumbnail(EomImage *img, GdkPixbuf *thumbnail) {
  EomImagePrivate *priv;
...
void eom_image_get_size(EomImage *img, int *width, int *height) {
  EomImagePrivate *priv;
...
static GFile *tmp_file_get(void) {
  GFile *tmp_file;
  char *tmp_file_path;
  gint fd;
...
static void transfer_progress_cb(goffset cur_bytes, goffset total_bytes,
                                 gpointer user_data) {
  EomImage *image = EOM_IMAGE(user_data);

  if (cur_bytes > 0) {
    g_signal_emit(image, signals[SIGNAL_SAVE_PROGRESS], 0,
                  (gfloat)cur_bytes / (gfloat)total_bytes);
  }
}

static void tmp_file_restore_unix_attributes(GFile *temp_file,
                                             GFile *target_file) {
  GFileInfo *file_info;
  guint uid;
  guint gid;
  ...
  file_info = g_file_query_info(target_file, "unix::uid,unix::gid,unix::mode",
  ...
  uid = g_file_info_get_attribute_uint32(file_info, G_FILE_ATTRIBUTE_UNIX_UID);

  gid = g_file_info_get_attribute_uint32(file_info, G_FILE_ATTRIBUTE_UNIX_GID);

  mode =
      g_file_info_get_attribute_uint32(file_info, G_FILE_ATTRIBUTE_UNIX_MODE);

  /* apply default mode mask to file mode */
  mode |= mode_mask;

  /* restore original UID, GID, and MODE into the temporal file */
  g_file_set_attribute_uint32(temp_file, G_FILE_ATTRIBUTE_UNIX_UID, uid,
                              G_FILE_QUERY_INFO_NONE, NULL, &error);
  ...
  g_file_set_attribute_uint32(temp_file, G_FILE_ATTRIBUTE_UNIX_GID, gid,
                              G_FILE_QUERY_INFO_NONE, NULL, &error);

  /* check that there aren't any error */
  if (error != NULL) {
    eom_debug_message(DEBUG_IMAGE_SAVE,
                      "You do not have the permissions necessary to change the "
                      "file GID. Setting user default GID.");

    g_clear_error(&error);
  }

  g_file_set_attribute_uint32(temp_file, G_FILE_ATTRIBUTE_UNIX_MODE, mode,
                              G_FILE_QUERY_INFO_NONE, NULL, &error);
...
static gboolean tmp_file_move_to_uri(EomImage *image, GFile *tmpfile,
  ...
  result = g_file_move(
      tmpfile, file,
      (overwrite ? G_FILE_COPY_OVERWRITE : 0) | G_FILE_COPY_ALL_METADATA, NULL,
      (GFileProgressCallback)transfer_progress_cb, image, &ioerror);

  if (result == FALSE) {
    if (g_error_matches(ioerror, G_IO_ERROR, G_IO_ERROR_EXISTS)) {
      g_set_error(error, EOM_IMAGE_ERROR, EOM_IMAGE_ERROR_FILE_EXISTS,
                  "File exists");
    } else {
      g_set_error(error, EOM_IMAGE_ERROR, EOM_IMAGE_ERROR_VFS,
...
static gboolean tmp_file_delete(GFile *tmpfile) {
  gboolean result;
  GError *err = NULL;
...
static void eom_image_reset_modifications(EomImage *image) {
  EomImagePrivate *priv;
...
static void eom_image_link_with_target(EomImage *image,
...
gboolean eom_image_save_by_info(EomImage *img, EomImageSaveInfo *source,
                                GError **error) {
  EomImagePrivate *priv;
  ...
  if (tmp_file == NULL) {
    g_set_error(error, EOM_IMAGE_ERROR, EOM_IMAGE_ERROR_TMP_FILE_FAILED,
  ...
  if (!success && (*error == NULL)) {
    success = gdk_pixbuf_save(priv->image, tmp_file_path, source->format, error,
                              NULL);
  }

  if (success) {
    /* try to move result file to target uri */
    success = tmp_file_move_to_uri(img, tmp_file, priv->file,
                                   TRUE /*overwrite*/, error);
  }
...
static gboolean eom_image_copy_file(EomImage *image, EomImageSaveInfo *source,
  ...
  result = g_file_copy(source->file, target->file,
                       (target->overwrite ? G_FILE_COPY_OVERWRITE : 0) |
                           G_FILE_COPY_ALL_METADATA,
                       NULL, EOM_IS_IMAGE(image) ? transfer_progress_cb : NULL,
                       image, &ioerror);

  if (result == FALSE) {
    if (ioerror->code == G_IO_ERROR_EXISTS) {
      g_set_error(error, EOM_IMAGE_ERROR, EOM_IMAGE_ERROR_FILE_EXISTS, "%s",
                  ioerror->message);
    } else {
      g_set_error(error, EOM_IMAGE_ERROR, EOM_IMAGE_ERROR_VFS, "%s",
                  ioerror->message);
    }
...
gboolean eom_image_save_as_by_info(EomImage *img, EomImageSaveInfo *source,
                                   EomImageSaveInfo *target, GError **error) {
  EomImagePrivate *priv;
  gboolean success = FALSE;
  char *tmp_file_path;
  ...
  /* determine kind of saving */
  if (g_ascii_strcasecmp(source->format, target->format) == 0 &&
      !source->modified) {
    success = eom_image_copy_file(img, source, target, error);
    direct_copy = success;
  }
#ifdef HAVE_JPEG
  else if ((g_ascii_strcasecmp(source->format, EOM_FILE_FORMAT_JPEG) == 0 &&
            source->exists) ||
           (g_ascii_strcasecmp(target->format, EOM_FILE_FORMAT_JPEG) == 0)) {
    success =
        eom_image_jpeg_save_file(img, tmp_file_path, source, target, error);
  }
#endif

  if (!success && (*error == NULL)) {
    success = gdk_pixbuf_save(priv->image, tmp_file_path, target->format, error,
                              NULL);
  }

  if (success &&
      !direct_copy) { /* not required if we alredy copied the file directly */
    /* try to move result file to target uri */
    success = tmp_file_move_to_uri(img, tmp_file, target->file,
                                   target->overwrite, error);
  }
...
const gchar *eom_image_get_caption(EomImage *img) {
  ...
  if (priv->caption != NULL) /* Use cached caption string */
    return priv->caption;
 ...
 * whether the metadata has not been read yet, is ready, or not available at
 *all.
 *
...
GList *eom_image_get_supported_mime_types(void) {
  ...
  if (supported_mime_types == NULL) {
    ...
    for (it = format_list; it != NULL; it = it->next) {
      ...
      for (i = 0; mime_types[i] != NULL; i++) {
        supported_mime_types =
            g_list_prepend(supported_mime_types, g_strdup(mime_types[i]));
      }
    ...
    supported_mime_types =
        g_list_sort(supported_mime_types, (GCompareFunc)compare_quarks);
...
gboolean eom_image_is_supported_mime_type(const char *mime_type) {
  ...
  result =
      g_list_find_custom(eom_image_get_supported_mime_types(),
                         GINT_TO_POINTER(quark), (GCompareFunc)compare_quarks);

  return (result != NULL);
}

static gboolean eom_image_iter_advance(EomImage *img) {
  EomImagePrivate *priv;
  gboolean new_frame;

  g_return_val_if_fail(EOM_IS_IMAGE(img), FALSE);
  g_return_val_if_fail(GDK_IS_PIXBUF_ANIMATION_ITER(img->priv->anim_iter),
                       FALSE);

  priv = img->priv;

  if ((new_frame = gdk_pixbuf_animation_iter_advance(img->priv->anim_iter,
                                                     NULL)) == TRUE) {
    g_mutex_lock(&priv->status_mutex);
    g_object_unref(priv->image);
    priv->image = gdk_pixbuf_animation_iter_get_pixbuf(priv->anim_iter);
    ...
    if (EOM_IS_TRANSFORM(priv->trans)) {
      GdkPixbuf *transformed =
          eom_transform_apply(priv->trans, priv->image, NULL);
      g_object_unref(priv->image);
      priv->image = transformed;
      priv->width = gdk_pixbuf_get_width(transformed);
      priv->height = gdk_pixbuf_get_height(transformed);
...
static gboolean private_timeout(gpointer data) {
  EomImage *img = EOM_IMAGE(data);
  EomImagePrivate *priv = img->priv;

  if (eom_image_is_animation(img) &&
      !g_source_is_destroyed(g_main_current_source()) && priv->is_playing) {
    while (eom_image_iter_advance(img) != TRUE) {
    }; /* cpu-sucking ? */
 ...
 *
 * Returns: %TRUE on success, %FALSE if @img is already playing or isn't an
 *animated image.
 **/
gboolean eom_image_start_animation(EomImage *img) {
  ...
  if (!eom_image_is_animation(img) || priv->is_playing) return FALSE;
