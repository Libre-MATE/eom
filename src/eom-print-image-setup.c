...
enum { PROP_0, PROP_IMAGE, PROP_PAGE_SETUP };

enum { CENTER_NONE, CENTER_HORIZONTAL, CENTER_VERTICAL, CENTER_BOTH };

enum { CHANGE_HORIZ, CHANGE_VERT };

enum { UNIT_INCH, UNIT_MM };
...
static void eom_print_image_setup_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec);
static void eom_print_image_setup_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *pspec);

static void on_left_value_changed(GtkSpinButton *spinbutton,
                                  gpointer user_data);
static void on_right_value_changed(GtkSpinButton *spinbutton,
                                   gpointer user_data);
static void on_top_value_changed(GtkSpinButton *spinbutton, gpointer user_data);
static void on_bottom_value_changed(GtkSpinButton *spinbutton,
                                    gpointer user_data);

static void on_width_value_changed(GtkSpinButton *spinbutton,
                                   gpointer user_data);
static void on_height_value_changed(GtkSpinButton *spinbutton,
                                    gpointer user_data);

G_DEFINE_TYPE_WITH_PRIVATE(EomPrintImageSetup, eom_print_image_setup,
                           GTK_TYPE_GRID);

static void block_handlers(EomPrintImageSetup *setup) {
...
static void unblock_handlers(EomPrintImageSetup *setup) {
  EomPrintImageSetupPrivate *priv = setup->priv;
  ...
  g_signal_handlers_unblock_by_func(priv->bottom, on_bottom_value_changed,
                                    setup);
  g_signal_handlers_unblock_by_func(priv->height, on_height_value_changed,
                                    setup);
}

static gdouble get_scale_to_px_factor(EomPrintImageSetup *setup) {
  gdouble factor = 0.;
...
static gdouble get_max_percentage(EomPrintImageSetup *setup) {
  EomPrintImageSetupPrivate *priv = setup->priv;
...
static void center(gdouble page_width, gdouble width, GtkSpinButton *s_left,
...
static void on_center_changed(GtkComboBox *combobox, gpointer user_data) {
...
static void update_image_pos_ranges(EomPrintImageSetup *setup,
  ...
  gtk_spin_button_set_range(GTK_SPIN_BUTTON(priv->right), 0,
                            page_width - width);
  gtk_spin_button_set_range(GTK_SPIN_BUTTON(priv->top), 0,
                            page_height - height);
  gtk_spin_button_set_range(GTK_SPIN_BUTTON(priv->bottom), 0,
                            page_height - height);
}
...
static gboolean on_scale_changed(GtkRange *range, gpointer user_data) {
  ...
  scale =
      CLAMP(0.01 * gtk_range_get_value(range), 0, get_max_percentage(setup));
...
static gchar *on_scale_format_value(GtkScale *scale, gdouble value) {
  return g_strdup_printf("%i%%", (gint)value);
}

static void position_values_changed(EomPrintImageSetup *setup,
                                    GtkWidget *w_changed,
                                    GtkWidget *w_to_update, GtkWidget *w_size,
                                    gdouble total_size, gint change) {
  EomPrintImageSetupPrivate *priv;
  ...
  switch (change) {
    case CHANGE_HORIZ:
      ...
      eom_print_preview_set_image_position(EOM_PRINT_PREVIEW(priv->preview),
                                           pos, -1);
      break;
    case CHANGE_VERT:
      ...
      eom_print_preview_set_image_position(EOM_PRINT_PREVIEW(priv->preview), -1,
                                           pos);
      break;
  }
}
...
static void on_left_value_changed(GtkSpinButton *spinbutton,
  ...
  position_values_changed(
      setup, priv->left, priv->right, priv->width,
      gtk_page_setup_get_page_width(priv->page_setup, priv->current_unit),
      CHANGE_HORIZ);
}
...
static void on_right_value_changed(GtkSpinButton *spinbutton,
  ...
  position_values_changed(
      EOM_PRINT_IMAGE_SETUP(user_data), priv->right, priv->left, priv->width,
      gtk_page_setup_get_page_width(priv->page_setup, priv->current_unit),
      CHANGE_HORIZ);
}

static void on_top_value_changed(GtkSpinButton *spinbutton,
                                 gpointer user_data) {
  EomPrintImageSetupPrivate *priv;
  ...
  position_values_changed(
      EOM_PRINT_IMAGE_SETUP(user_data), priv->top, priv->bottom, priv->height,
      gtk_page_setup_get_page_height(priv->page_setup, priv->current_unit),
      CHANGE_VERT);
}
...
static void on_bottom_value_changed(GtkSpinButton *spinbutton,
  ...
  position_values_changed(
      EOM_PRINT_IMAGE_SETUP(user_data), priv->bottom, priv->top, priv->height,
      gtk_page_setup_get_page_height(priv->page_setup, priv->current_unit),
      CHANGE_VERT);
}

static void size_changed(EomPrintImageSetup *setup, GtkWidget *w_size_x,
                         GtkWidget *w_size_y, GtkWidget *w_margin_x_1,
                         GtkWidget *w_margin_x_2, GtkWidget *w_margin_y_1,
                         GtkWidget *w_margin_y_2, gdouble page_size_x,
                         gdouble page_size_y, gint change) {
  EomPrintImageSetupPrivate *priv;
...
static void on_width_value_changed(GtkSpinButton *spinbutton,
  ...
  size_changed(
      EOM_PRINT_IMAGE_SETUP(user_data), priv->width, priv->height, priv->left,
      priv->right, priv->top, priv->bottom,
      gtk_page_setup_get_page_width(priv->page_setup, priv->current_unit),
      gtk_page_setup_get_page_height(priv->page_setup, priv->current_unit),
      CHANGE_HORIZ);
}
...
static void on_height_value_changed(GtkSpinButton *spinbutton,
  ...
  size_changed(
      EOM_PRINT_IMAGE_SETUP(user_data), priv->height, priv->width, priv->top,
...
static void change_unit(GtkSpinButton *spinbutton, gdouble factor, gint digits,
...
static void set_scale_unit(EomPrintImageSetup *setup, GtkUnit unit) {
  EomPrintImageSetupPrivate *priv = setup->priv;
  gdouble factor;
  gdouble step, page;
  gint digits;

  if (G_UNLIKELY(priv->current_unit == unit)) return;
...
static void on_unit_changed(GtkComboBox *combobox, gpointer user_data) {
...
static void on_preview_image_moved(EomPrintPreview *preview,
                                   gpointer user_data) {
  EomPrintImageSetupPrivate *priv = EOM_PRINT_IMAGE_SETUP(user_data)->priv;
  gdouble x, y;
...
static GtkWidget *wrap_in_frame(const gchar *label, GtkWidget *child) {
...
static GtkWidget *grid_attach_spin_button_with_label(GtkWidget *grid,
  ...
  gtk_grid_attach_next_to(GTK_GRID(grid), spin_button, label, GTK_POS_RIGHT, 1,
                          1);
  ...
  switch (prop_id) {
    case PROP_IMAGE:
      ...
      if (EOM_IS_IMAGE(priv->image)) {
        ...
        g_object_set(priv->preview, "image", pixbuf, NULL);
...
static void set_initial_values(EomPrintImageSetup *setup) {
  EomPrintImageSetupPrivate *priv;
  GtkPageSetup *page_setup;
  EomImage *image;
  gdouble page_width, page_height;
...
static void connect_signals(EomPrintImageSetup *setup) {
  EomPrintImageSetupPrivate *priv;
  ...
  g_signal_connect(priv->top, "value-changed", G_CALLBACK(on_top_value_changed),
                   setup);
  ...
  g_signal_connect(priv->scaling, "value-changed", G_CALLBACK(on_scale_changed),
                   setup);
...
static void eom_print_image_setup_class_init(EomPrintImageSetupClass *class) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          ...
          EOM_TYPE_IMAGE, G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_PAGE_SETUP,
      g_param_spec_object(
          "page-setup", _("Page Setup"),
          _("The information for the page where the image will be printed"),
          GTK_TYPE_PAGE_SETUP, G_PARAM_READWRITE));
}

static void eom_print_image_setup_init(EomPrintImageSetup *setup) {
  GtkWidget *frame;
  GtkWidget *grid;
  GtkWidget *label;
  GtkWidget *hscale;
  ...
  gtk_combo_box_text_insert_text(GTK_COMBO_BOX_TEXT(combobox), CENTER_NONE,
                                 _("None"));
  gtk_combo_box_text_insert_text(GTK_COMBO_BOX_TEXT(combobox),
                                 CENTER_HORIZONTAL, _("Horizontal"));
  gtk_combo_box_text_insert_text(GTK_COMBO_BOX_TEXT(combobox), CENTER_VERTICAL,
                                 _("Vertical"));
  gtk_combo_box_text_insert_text(GTK_COMBO_BOX_TEXT(combobox), CENTER_BOTH,
                                 _("Both"));
  ...
  gtk_grid_attach_next_to(GTK_GRID(grid), combobox, priv->right, GTK_POS_BOTTOM,
                          3, 1);
  ...
  g_signal_connect(combobox, "changed", G_CALLBACK(on_center_changed), setup);
  ...
  gtk_grid_attach_next_to(GTK_GRID(grid), hscale, priv->width, GTK_POS_BOTTOM,
                          3, 1);
  ...
  gtk_grid_attach_next_to(GTK_GRID(grid), combobox, hscale, GTK_POS_BOTTOM, 3,
                          1);
  gtk_grid_attach_next_to(GTK_GRID(grid), label, combobox, GTK_POS_LEFT, 1, 1);
  gtk_label_set_mnemonic_widget(GTK_LABEL(label), combobox);
  priv->unit = combobox;
  g_signal_connect(combobox, "changed", G_CALLBACK(on_unit_changed), setup);
...
GtkWidget *eom_print_image_setup_new(EomImage *image,
  ...
  setup = g_object_new(EOM_TYPE_PRINT_IMAGE_SETUP, "orientation",
                       GTK_ORIENTATION_VERTICAL, "row-spacing", 18,
                       "column-spacing", 18, "border-width", 12, "image", image,
                       "page-setup", page_setup, NULL);

  set_initial_values(EOM_PRINT_IMAGE_SETUP(setup));

  preview = EOM_PRINT_IMAGE_SETUP(setup)->priv->preview;
  eom_print_preview_set_from_page_setup(EOM_PRINT_PREVIEW(preview), page_setup);
 ...
 * @unit: a pointer where to store the #GtkUnit used by the @left and @top
 *values.
 *
 * Gets the options set by the #EomPrintImageSetup.
 **/
void eom_print_image_setup_get_options(EomPrintImageSetup *setup, gdouble *left,
                                       gdouble *top, gdouble *scale,
                                       GtkUnit *unit) {
  EomPrintImageSetupPrivate *priv;
...
void eom_print_image_setup_update(GtkPrintOperation *operation,
  ...
  eom_print_preview_set_image_position(EOM_PRINT_PREVIEW(setup->priv->preview),
                                       pos_x, pos_y);
}
