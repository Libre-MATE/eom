...
static char *set_row_data(GtkTreeStore *store, char *path, char *parent,
                          const char *attribute, const char *value);

static void eom_metadata_details_reset(EomMetadataDetails *exif_details);

G_DEFINE_TYPE_WITH_PRIVATE(EomMetadataDetails, eom_metadata_details,
                           GTK_TYPE_TREE_VIEW)

static void eom_metadata_details_dispose(GObject *object) {
...
static void eom_metadata_details_init(EomMetadataDetails *details) {
  EomMetadataDetailsPrivate *priv;
  ...
  priv->id_path_hash_mnote =
      g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, g_free);

  /* Tag name column */
  cell = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      _("Tag"), cell, "text", MODEL_COLUMN_ATTRIBUTE, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(details), column);

  /* Value column */
  cell = gtk_cell_renderer_text_new();
  g_object_set(cell, "editable", TRUE, NULL);
  column = gtk_tree_view_column_new_with_attributes(_("Value"), cell, "text",
                                                    MODEL_COLUMN_VALUE, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(details), column);

  eom_metadata_details_reset(details);

  gtk_tree_view_set_model(GTK_TREE_VIEW(details), GTK_TREE_MODEL(priv->model));
}

static void eom_metadata_details_class_init(EomMetadataDetailsClass *klass) {
...
#ifdef HAVE_EXIF
static MetadataCategory get_exif_category(ExifEntry *entry) {
...
static const char *eom_exif_entry_get_value(ExifEntry *e, char *buf,
                                            guint n_buf) {
  ExifByteOrder bo;
  ...
  switch ((guint16)e->tag) {
    ...
    case EXIF_TAG_GPS_LONGITUDE: {
      ...
      if (G_UNLIKELY(e->components != 3 || e->format != EXIF_FORMAT_RATIONAL))
        return exif_entry_get_value(e, buf, n_buf);

      r = exif_get_rational(e->data, bo);
      if (r.denominator != 0) h = (gfloat)r.numerator / r.denominator;
      ...
      r = exif_get_rational(e->data + (2 * rational_size), bo);
      if (r.denominator != 0) {
        if (r.numerator != 0) {
      ...
      if (s != 0.0) {
        g_snprintf(buf, n_buf, "%.0f° %.0f' %.2f\"", h, m, s);
      } else {
        g_snprintf(buf, n_buf, "%.0f° %.2f'", h, m);
      }
    ...
    case EXIF_TAG_GPS_LONGITUDE_REF: {
      if (G_UNLIKELY(e->components != 2 || e->format != EXIF_FORMAT_ASCII))
        return exif_entry_get_value(e, buf, n_buf);
...
static void exif_entry_cb(ExifEntry *entry, gpointer data) {
  GtkTreeStore *store;
  ...
  if (path != NULL) {
    set_row_data(store, path, NULL, exif_tag_get_name_in_ifd(entry->tag, ifd),
                 eom_exif_entry_get_value(entry, b, sizeof(b)));
  } else {
    ExifMnoteData *mnote =
        (entry->tag == EXIF_TAG_MAKER_NOTE
             ? exif_data_get_mnote_data(entry->parent->parent)
             : NULL);
    ...
    if (mnote) {
      ...
      for (i = 0; i < c; i++) {
        path =
            g_hash_table_lookup(priv->id_path_hash_mnote, GINT_TO_POINTER(i));
        if (path != NULL) {
          set_row_data(store, path, NULL, exif_mnote_data_get_title(mnote, i),
                       exif_mnote_data_get_value(mnote, i, b, sizeof(b)));
        } else {
          path = set_row_data(
              store, NULL, exif_categories[EXIF_CATEGORY_MAKER_NOTE].path,
              exif_mnote_data_get_title(mnote, i),
              exif_mnote_data_get_value(mnote, i, b, sizeof(b)));
          g_hash_table_insert(priv->id_path_hash_mnote, GINT_TO_POINTER(i),
                              path);
        }
      }
    } else {
      ...
      g_hash_table_insert(priv->id_path_hash, GINT_TO_POINTER(key), path);
    }
  }
}
#endif
...
#ifdef HAVE_EXIF
static void exif_content_cb(ExifContent *content, gpointer data) {
...
static void eom_metadata_details_reset(EomMetadataDetails *details) {
...
static MetadataCategory get_xmp_category(XmpStringPtr schema) {
...
static void xmp_entry_insert(EomMetadataDetails *view, XmpStringPtr xmp_schema,
  ...
  key = g_strconcat(xmp_string_cstr(xmp_schema), ":", xmp_string_cstr(xmp_path),
                    NULL);
  ...
  if (path != NULL) {
    set_row_data(store, path, NULL, xmp_string_cstr(xmp_path),
    ...
    path = set_row_data(store, NULL, exif_categories[cat].path,
                        xmp_string_cstr(xmp_path), xmp_string_cstr(xmp_prop));

    g_hash_table_insert(priv->id_path_hash, key, path);
  }
}

void eom_metadata_details_xmp_update(EomMetadataDetails *view, XmpPtr data) {
  g_return_if_fail(EOM_IS_METADATA_DETAILS(view));

  if (data) {
    XmpIteratorPtr iter =
        xmp_iterator_new(data, NULL, NULL, XMP_ITER_JUSTLEAFNODES);
