...
enum { PROP_0, PROP_THUMBVIEW, PROP_NETBOOK_MODE };
...
G_DEFINE_TYPE_WITH_PRIVATE(EomPropertiesDialog, eom_properties_dialog,
                           GTK_TYPE_DIALOG);

static void parent_file_display_name_query_info_cb(GObject *source_object,
                                                   GAsyncResult *res,
                                                   gpointer user_data) {
  ...
  gtk_button_set_label(GTK_BUTTON(prop_dlg->priv->folder_button), display_name);
...
static void pd_update_general_tab(EomPropertiesDialog *prop_dlg,
                                  EomImage *image) {
  ...
  g_object_set(G_OBJECT(prop_dlg->priv->thumbnail_image), "pixbuf",
               eom_image_get_thumbnail(image), NULL);
  ...
  width_str =
      g_strdup_printf("%d %s", width, ngettext("pixel", "pixels", width));
  height_str =
      g_strdup_printf("%d %s", height, ngettext("pixel", "pixels", height));

  gtk_label_set_text(GTK_LABEL(prop_dlg->priv->width_label), width_str);

  gtk_label_set_text(GTK_LABEL(prop_dlg->priv->height_label), height_str);
  ...
  g_file_query_info_async(parent_file, G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME,
...
#if HAVE_EXEMPI
static void eom_xmp_set_label(XmpPtr xmp, const char *ns, const char *propname,
                              GtkWidget *w) {
  uint32_t options;
  ...
  if (xmp_get_property(xmp, ns, propname, value, &options)) {
    if (XMP_IS_PROP_SIMPLE(options)) {
      ...
      XmpIteratorPtr iter =
          xmp_iterator_new(xmp, ns, propname, XMP_ITER_JUSTLEAFNODES);
...
#if HAVE_METADATA
static void pd_update_metadata_tab(EomPropertiesDialog *prop_dlg,
                                   EomImage *image) {
  ...
  if (TRUE
    ...
    } else if (gtk_notebook_get_current_page(notebook) ==
  ...
  eom_exif_util_set_label_text(GTK_LABEL(priv->exif_aperture_label), exif_data,
                               EXIF_TAG_FNUMBER);

  eom_exif_util_set_label_text(GTK_LABEL(priv->exif_exposure_label), exif_data,
                               EXIF_TAG_EXPOSURE_TIME);

  eom_exif_util_set_focal_length_label_text(GTK_LABEL(priv->exif_focal_label),
                                            exif_data);

  eom_exif_util_set_label_text(GTK_LABEL(priv->exif_flash_label), exif_data,
                               EXIF_TAG_FLASH);

  eom_exif_util_set_label_text(GTK_LABEL(priv->exif_iso_label), exif_data,
                               EXIF_TAG_ISO_SPEED_RATINGS);

  eom_exif_util_set_label_text(GTK_LABEL(priv->exif_metering_label), exif_data,
                               EXIF_TAG_METERING_MODE);

  eom_exif_util_set_label_text(GTK_LABEL(priv->exif_model_label), exif_data,
                               EXIF_TAG_MODEL);

  eom_exif_util_set_label_text(GTK_LABEL(priv->exif_date_label), exif_data,
  ...
  if (xmp_data != NULL) {
    eom_xmp_set_label(xmp_data, NS_IPTC4XMP, "Location",
                      priv->xmp_location_label);

    eom_xmp_set_label(xmp_data, NS_DC, "description",
                      priv->xmp_description_label);

    eom_xmp_set_label(xmp_data, NS_DC, "subject", priv->xmp_keywords_label);

    eom_xmp_set_label(xmp_data, NS_DC, "creator", priv->xmp_creator_label);

    eom_xmp_set_label(xmp_data, NS_DC, "rights", priv->xmp_rights_label);

    eom_metadata_details_xmp_update(
        EOM_METADATA_DETAILS(priv->metadata_details), xmp_data);
    ...
    gtk_label_set_text(GTK_LABEL(priv->xmp_description_label), NULL);
...
static gboolean pd_resize_dialog(gpointer user_data) {
  gint width, height;
...
static void pd_exif_details_activated_cb(GtkExpander *expander,
  ...
  if (!expanded) g_timeout_add(150, pd_resize_dialog, dialog);
}
#endif

static void pd_folder_button_clicked_cb(GtkButton *button, gpointer data) {
  EomPropertiesDialogPrivate *priv = EOM_PROPERTIES_DIALOG(data)->priv;
  GtkWindow *window;
  guint32 timestamp;

  if (!priv->folder_button_uri) return;
...
static gboolean eom_properties_dialog_page_switch(
    GtkNotebook *notebook, gpointer page, guint page_index,
    EomPropertiesDialog *prop_dlg) {
  if (prop_dlg->priv->update_page) prop_dlg->priv->current_page = page_index;
...
void eom_properties_dialog_set_netbook_mode(EomPropertiesDialog *dlg,
  ...
  if (priv->netbook_mode == enable) return;
  ...
  if (enable) {
    ...
    gtk_container_remove(
        GTK_CONTAINER(gtk_widget_get_parent(priv->metadata_details_sw)),
        priv->metadata_details_sw);
    gtk_container_add(GTK_CONTAINER(priv->metadata_details_box),
                      priv->metadata_details_sw);
...
static void eom_properties_dialog_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec) {
...
static void eom_properties_dialog_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *pspec) {
  EomPropertiesDialog *prop_dlg = EOM_PROPERTIES_DIALOG(object);
  ...
  switch (prop_id) {
    case PROP_THUMBVIEW:
      ...
      break;
    case PROP_NETBOOK_MODE:
      g_value_set_boolean(value, prop_dlg->priv->netbook_mode);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
      break;
...
static void eom_properties_dialog_dispose(GObject *object) {
  EomPropertiesDialog *prop_dlg;
  EomPropertiesDialogPrivate *priv;
...
static void eom_properties_dialog_class_init(EomPropertiesDialogClass *klass) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_THUMBVIEW,
      g_param_spec_object(
          "thumbview", "Thumbview", "Thumbview", EOM_TYPE_THUMB_VIEW,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME |
              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
  g_object_class_install_property(
      g_object_class, PROP_NETBOOK_MODE,
      g_param_spec_boolean("netbook-mode", "Netbook Mode", "Netbook Mode",
                           FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  GtkWidgetClass *wklass = (GtkWidgetClass *)klass;

  gtk_widget_class_set_template_from_resource(
      wklass, "/org/mate/eom/ui/eom-image-properties-dialog.ui");

  gtk_widget_class_bind_template_child_private(wklass, EomPropertiesDialog,
                                               notebook);
  gtk_widget_class_bind_template_child_private(wklass, EomPropertiesDialog,
                                               previous_button);
  gtk_widget_class_bind_template_child_private(wklass, EomPropertiesDialog,
  ...
  gtk_widget_class_bind_template_callback(wklass, pd_exif_details_activated_cb);
#endif
  gtk_widget_class_bind_template_callback(wklass,
                                          eom_properties_dialog_page_switch);
  gtk_widget_class_bind_template_callback(wklass, pd_folder_button_clicked_cb);
}

static void eom_properties_dialog_init(EomPropertiesDialog *prop_dlg) {
  EomPropertiesDialogPrivate *priv;
#if HAVE_METADATA
  GtkWidget *sw;
#endif
  ...
  g_signal_connect(prop_dlg, "delete-event",
                   G_CALLBACK(gtk_widget_hide_on_delete), prop_dlg);
  ...
  if (priv->netbook_mode) {
    gtk_widget_hide(priv->metadata_details_expander);
    gtk_box_pack_start(GTK_BOX(priv->metadata_details_box), sw, TRUE, TRUE, 6);
  } else {
    gtk_container_add(GTK_CONTAINER(priv->metadata_details_expander), sw);
  }
  ...
  g_signal_connect(priv->notebook, "scroll-event",
                   G_CALLBACK(eom_notebook_scroll_event_cb), NULL);
}
 ...
 *
 *
 * Returns: (transfer full) (type EomPropertiesDialog): a new
 *#EomPropertiesDialog
 **/
...
GtkWidget *eom_properties_dialog_new(GtkWindow *parent, EomThumbView *thumbview,
  ...
  prop_dlg =
      g_object_new(EOM_TYPE_PROPERTIES_DIALOG, "thumbview", thumbview, NULL);
  ...
  gtk_activatable_set_related_action(
      GTK_ACTIVATABLE(EOM_PROPERTIES_DIALOG(prop_dlg)->priv->next_button),
      next_image_action);

  gtk_activatable_set_related_action(
      GTK_ACTIVATABLE(EOM_PROPERTIES_DIALOG(prop_dlg)->priv->previous_button),
      previous_image_action);

  return GTK_WIDGET(prop_dlg);
}

void eom_properties_dialog_update(EomPropertiesDialog *prop_dlg,
                                  EomImage *image) {
  g_return_if_fail(EOM_IS_PROPERTIES_DIALOG(prop_dlg));
...
void eom_properties_dialog_set_page(EomPropertiesDialog *prop_dlg,
                                    EomPropertiesDialogPage page) {
  g_return_if_fail(EOM_IS_PROPERTIES_DIALOG(prop_dlg));

  prop_dlg->priv->current_page = page;

  gtk_notebook_set_current_page(GTK_NOTEBOOK(prop_dlg->priv->notebook), page);
}
