...
#define EOM_IMAGE(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), EOM_TYPE_IMAGE, EomImage))
#define EOM_IMAGE_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_CAST((klass), EOM_TYPE_IMAGE, EomImageClass))
#define EOM_IS_IMAGE(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), EOM_TYPE_IMAGE))
#define EOM_IS_IMAGE_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_TYPE((klass), EOM_TYPE_IMAGE))
#define EOM_IMAGE_GET_CLASS(obj) \
  (G_TYPE_INSTANCE_GET_CLASS((obj), EOM_TYPE_IMAGE, EomImageClass))
...
struct _EomImageClass {
  ...
  void (*size_prepared)(EomImage *img, int width, int height);

  void (*thumbnail_changed)(EomImage *img);

  void (*save_progress)(EomImage *img, gfloat progress);

  void (*next_frame)(EomImage *img, gint delay);
...
gboolean eom_image_has_data(EomImage *img, EomImageData data);

void eom_image_data_ref(EomImage *img);

void eom_image_data_unref(EomImage *img);

void eom_image_set_thumbnail(EomImage *img, GdkPixbuf *pixbuf);
