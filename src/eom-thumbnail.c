...
static GQuark eom_thumb_error_quark(void) {
  static GQuark q = 0;
  if (q == 0) q = g_quark_from_static_string("eom-thumb-error-quark");

  return q;
}

static void set_vfs_error(GError **error, GError *ioerror) {
  g_set_error(error, EOM_THUMB_ERROR, EOM_THUMB_ERROR_VFS, "%s",
              ioerror ? ioerror->message : "VFS error making a thumbnail");
}

static void set_thumb_error(GError **error, int error_id, const char *string) {
  g_set_error(error, EOM_THUMB_ERROR, error_id, "%s", string);
}

static GdkPixbuf *get_valid_thumbnail(EomThumbData *data, GError **error) {
  ...
  if (data->thumb_exists) {
    ...
    if (thumb != NULL &&
        !mate_desktop_thumbnail_is_valid(thumb, data->uri_str, data->mtime)) {
...
static GdkPixbuf *create_thumbnail_from_pixbuf(EomThumbData *data,
                                               GdkPixbuf *pixbuf,
                                               GError **error) {
  GdkPixbuf *thumb;
  gint width, height;
  ...
  thumb = gdk_pixbuf_scale_simple(pixbuf, width * perc, height * perc,
                                  GDK_INTERP_HYPER);

  return thumb;
}

static void eom_thumb_data_free(EomThumbData *data) {
  if (data == NULL) return;
...
static EomThumbData *eom_thumb_data_new(GFile *file, GError **error) {
  EomThumbData *data;
  GFileInfo *file_info;
  GError *ioerror = NULL;
  ...
  data->thumb_path = mate_desktop_thumbnail_path_for_uri(
      data->uri_str, MATE_DESKTOP_THUMBNAIL_SIZE_NORMAL);
  ...
  if (*error == NULL) {
    ...
    data->mtime = g_file_info_get_attribute_uint64(
        file_info, G_FILE_ATTRIBUTE_TIME_MODIFIED);
    data->mime_type = g_strdup(g_file_info_get_content_type(file_info));

    data->thumb_exists =
        (g_file_info_get_attribute_byte_string(
             file_info, G_FILE_ATTRIBUTE_THUMBNAIL_PATH) != NULL);
    data->failed_thumb_exists = g_file_info_get_attribute_boolean(
        file_info, G_FILE_ATTRIBUTE_THUMBNAILING_FAILED);
    data->can_read = TRUE;
    if (g_file_info_has_attribute(file_info,
                                  G_FILE_ATTRIBUTE_ACCESS_CAN_READ)) {
      data->can_read = g_file_info_get_attribute_boolean(
...
static GdkPixbuf *eom_thumbnail_stretch_frame_image(
  ...
  draw_frame_row(frame_image, target_width, target_frame_width, 0, 0,
...
GdkPixbuf *eom_thumbnail_add_frame(GdkPixbuf *thumbnail) {
  ...
  result_pixbuf = eom_thumbnail_stretch_frame_image(
      frame, 3, 3, 6, 6, dest_width, dest_height, FALSE);

  gdk_pixbuf_copy_area(thumbnail, 0, 0, source_width, source_height,
                       result_pixbuf, 3, 3);
...
GdkPixbuf *eom_thumbnail_load(EomImage *image, GError **error) {
  ...
  if (!data->can_read ||
      (data->failed_thumb_exists &&
       mate_desktop_thumbnail_factory_has_valid_failed_thumbnail(
           factory, data->uri_str, data->mtime))) {
    eom_debug_message(DEBUG_THUMBNAIL,
                      "%s: bad permissions or valid failed thumbnail present",
                      data->uri_str);
    set_thumb_error(error, EOM_THUMB_ERROR_GENERIC,
  ...
  } else if (mate_desktop_thumbnail_factory_can_thumbnail(
                 factory, data->uri_str, data->mime_type, data->mtime)) {
    /* Only use the image pixbuf when it is up to date. */
    if (!eom_image_is_file_changed(image)) pixbuf = eom_image_get_pixbuf(image);

    if (pixbuf != NULL) {
      /* generate a thumbnail from the in-memory image,
         if we have already loaded the image */
      eom_debug_message(DEBUG_THUMBNAIL, "%s: creating from pixbuf",
                        data->uri_str);
      thumb = create_thumbnail_from_pixbuf(data, pixbuf, error);
      g_object_unref(pixbuf);
    } else {
      /* generate a thumbnail from the file */
      eom_debug_message(DEBUG_THUMBNAIL, "%s: creating from file",
                        data->uri_str);
      thumb = mate_desktop_thumbnail_factory_generate_thumbnail(
          factory, data->uri_str, data->mime_type);
    }

    if (thumb != NULL) {
      /* Save the new thumbnail */
      mate_desktop_thumbnail_factory_save_thumbnail(factory, thumb,
                                                    data->uri_str, data->mtime);
      eom_debug_message(DEBUG_THUMBNAIL, "%s: normal thumbnail saved",
                        data->uri_str);
    } else {
      /* Save a failed thumbnail, to stop further thumbnail attempts */
      mate_desktop_thumbnail_factory_create_failed_thumbnail(
          factory, data->uri_str, data->mtime);
      eom_debug_message(DEBUG_THUMBNAIL, "%s: failed thumbnail saved",
                        data->uri_str);
      set_thumb_error(error, EOM_THUMB_ERROR_GENERIC,
                      "Thumbnail creation failed");
    }
...
void eom_thumbnail_init(void) {
  ...
  if (frame == NULL) {
    frame = gdk_pixbuf_new_from_resource(
        "/org/mate/eom/ui/pixmaps/thumbnail-frame.png", NULL);
  }
}
