...
static void set_message_area_text_and_icon(GtkInfoBar *message_area,
  ...
  if (secondary_text != NULL) {
    secondary_markup =
        g_markup_printf_escaped("<small>%s</small>", secondary_text);
  ...
  gtk_box_pack_start(
      GTK_BOX(gtk_info_bar_get_content_area(GTK_INFO_BAR(message_area))),
      hbox_content, TRUE, TRUE, 0);
}

static GtkWidget *create_error_message_area(const gchar *primary_text,
                                            const gchar *secondary_text,
  ...
  if (recoverable)
    message_area =
        gtk_info_bar_new_with_buttons(_("_Retry"), GTK_RESPONSE_OK, NULL);
  else
    message_area = gtk_info_bar_new();

  gtk_info_bar_set_message_type(GTK_INFO_BAR(message_area), GTK_MESSAGE_ERROR);

  set_message_area_text_and_icon(GTK_INFO_BAR(message_area), "dialog-error",
...
GtkWidget *eom_image_load_error_message_area_new(const gchar *caption,
  ...
  error_message =
      g_strdup_printf(_("Could not load image '%s'."), pango_escaped_caption);

  message_details = eom_util_make_valid_utf8(error->message);

  message_area =
      create_error_message_area(error_message, message_details, TRUE);
...
GtkWidget *eom_no_images_error_message_area_new(GFile *file) {
  ...
  if (file != NULL) {
    ...
    error_message =
        g_strdup_printf(_("No images found in '%s'."), pango_escaped_str);
  ...
  message_area = create_error_message_area(error_message, NULL, FALSE);
