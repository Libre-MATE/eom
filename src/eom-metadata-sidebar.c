...
enum { PROP_0, PROP_IMAGE, PROP_PARENT_WINDOW };
...
G_DEFINE_TYPE_WITH_PRIVATE(EomMetadataSidebar, eom_metadata_sidebar,
...
static void parent_file_display_name_query_info_cb(GObject *source_object,
  ...
  str = g_markup_printf_escaped("<a href=\"%s\">%s</a>", baseuri, display_name);
...
static void eom_metadata_sidebar_update_general_section(
  ...
  str = g_strdup_printf(ngettext("%i × %i pixel", "%i × %i pixels", height),
                        width, height);
...
#if HAVE_METADATA
static void eom_metadata_sidebar_update_metadata_section(
    EomMetadataSidebar *sidebar) {
  EomMetadataSidebarPrivate *priv = sidebar->priv;
#if HAVE_EXIF
  EomImage *img = priv->image;
  ...
  eom_exif_util_set_label_text(GTK_LABEL(priv->aperture_label), exif_data,
                               EXIF_TAG_FNUMBER);
  eom_exif_util_set_label_text(GTK_LABEL(priv->exposure_label), exif_data,
                               EXIF_TAG_EXPOSURE_TIME);
  eom_exif_util_set_focal_length_label_text(GTK_LABEL(priv->focallen_label),
                                            exif_data);
  eom_exif_util_set_label_text(GTK_LABEL(priv->iso_label), exif_data,
                               EXIF_TAG_ISO_SPEED_RATINGS);
  eom_exif_util_set_label_text(GTK_LABEL(priv->metering_label), exif_data,
                               EXIF_TAG_METERING_MODE);
  eom_exif_util_set_label_text(GTK_LABEL(priv->model_label), exif_data,
                               EXIF_TAG_MODEL);
  eom_exif_util_format_datetime_label(GTK_LABEL(priv->date_label), exif_data,
                                      EXIF_TAG_DATE_TIME_ORIGINAL,
                                      _("%a, %d %B %Y"));
  eom_exif_util_format_datetime_label(GTK_LABEL(priv->time_label), exif_data,
                                      EXIF_TAG_DATE_TIME_ORIGINAL, _("%X"));
...
static void eom_metadata_sidebar_update(EomMetadataSidebar *sidebar) {
...
static void _thumbnail_changed_cb(EomImage *image, gpointer user_data) {
  eom_metadata_sidebar_update(EOM_METADATA_SIDEBAR(user_data));
}

static void eom_metadata_sidebar_set_image(EomMetadataSidebar *sidebar,
                                           EomImage *image) {
  EomMetadataSidebarPrivate *priv = sidebar->priv;

  if (image == priv->image) return;

#if GLIB_CHECK_VERSION(2, 62, 0)
  g_clear_signal_handler(&priv->thumb_changed_id, priv->image);
#else
  if (priv->thumb_changed_id != 0) {
    g_signal_handler_disconnect(priv->image, priv->thumb_changed_id);
  ...
  if (priv->image) {
    ...
    priv->thumb_changed_id =
        g_signal_connect(priv->image, "thumbnail-changed",
                         G_CALLBACK(_thumbnail_changed_cb), sidebar);
...
static void _notify_image_cb(GObject *gobject, GParamSpec *pspec,
                             gpointer user_data) {
  EomImage *image;
  ...
  eom_metadata_sidebar_set_image(EOM_METADATA_SIDEBAR(user_data), image);

  if (image) g_object_unref(image);
}

static void _folder_label_clicked_cb(GtkLabel *label, const gchar *uri,
                                     gpointer user_data) {
...
#ifdef HAVE_METADATA
static void _details_button_clicked_cb(GtkButton *button, gpointer user_data) {
  EomMetadataSidebarPrivate *priv = EOM_METADATA_SIDEBAR(user_data)->priv;
  GtkWidget *dlg;

  g_return_if_fail(priv->parent_window != NULL);

  dlg = eom_window_get_properties_dialog(EOM_WINDOW(priv->parent_window));
...
static void eom_metadata_sidebar_set_parent_window(EomMetadataSidebar *sidebar,
                                                   EomWindow *window) {
  ...
  priv->image_changed_id = g_signal_connect(
      view, "notify::image", G_CALLBACK(_notify_image_cb), sidebar);

  g_object_notify(G_OBJECT(sidebar), "parent-window");
}

static void eom_metadata_sidebar_init(EomMetadataSidebar *sidebar) {
  EomMetadataSidebarPrivate *priv;
...
static void eom_metadata_sidebar_get_property(GObject *object,
...
static void eom_metadata_sidebar_set_property(GObject *object,
                                              guint property_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
  EomMetadataSidebar *sidebar;
...
static void eom_metadata_sidebar_class_init(EomMetadataSidebarClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "parent-window", NULL, NULL, EOM_TYPE_WINDOW,
  ...
  gtk_widget_class_set_template_from_resource(
      widget_class, "/org/mate/eom/ui/metadata-sidebar.ui");

  gtk_widget_class_bind_template_child_private(widget_class, EomMetadataSidebar,
                                               size_label);
  gtk_widget_class_bind_template_child_private(widget_class, EomMetadataSidebar,
                                               type_label);
  gtk_widget_class_bind_template_child_private(widget_class, EomMetadataSidebar,
...
GtkWidget *eom_metadata_sidebar_new(EomWindow *window) {
  return gtk_widget_new(EOM_TYPE_METADATA_SIDEBAR, "hadjustment", NULL,
                        "vadjustment", NULL, "hscrollbar-policy",
                        GTK_POLICY_NEVER, "vscrollbar-policy",
                        GTK_POLICY_AUTOMATIC, "parent-window", window, NULL);
}
