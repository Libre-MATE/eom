...
enum { SIGNAL_FINISHED, SIGNAL_PROGRESS, SIGNAL_LAST_SIGNAL };
...
static void eom_job_init(EomJob *job) {
  /* NOTE: We need to allocate the mutex here so the ABI stays the same when it
   * used to use g_mutex_new */
  job->mutex = g_malloc(sizeof(GMutex));
  g_mutex_init(job->mutex);
  job->progress = 0.0;
}

static void eom_job_dispose(GObject *object) {
  EomJob *job;
...
static void eom_job_class_init(EomJobClass *class) {
  ...
  job_signals[SIGNAL_FINISHED] =
      g_signal_new("finished", EOM_TYPE_JOB, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EomJobClass, finished), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  job_signals[SIGNAL_PROGRESS] = g_signal_new(
      "progress", EOM_TYPE_JOB, G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EomJobClass, progress), NULL, NULL,
...
static gboolean notify_progress(gpointer data) {
...
void eom_job_set_progress(EomJob *job, float progress) {
  g_return_if_fail(EOM_IS_JOB(job));
...
static void eom_job_thumbnail_dispose(GObject *object) {
  EomJobThumbnail *job;
...
static void eom_job_thumbnail_class_init(EomJobThumbnailClass *class) {
  GObjectClass *oclass;
...
static void eom_job_thumbnail_run(EomJob *ejob) {
  ...
  job->thumbnail = eom_thumbnail_load(job->image, &ejob->error);
  ...
  orig_width = g_strdup(
      gdk_pixbuf_get_option(job->thumbnail, "tEXt::Thumb::Image::Width"));
  orig_height = g_strdup(
      gdk_pixbuf_get_option(job->thumbnail, "tEXt::Thumb::Image::Height"));
  ...
  if (orig_width) {
    ...
    g_object_set_data(G_OBJECT(job->thumbnail), EOM_THUMBNAIL_ORIGINAL_WIDTH,
                      GINT_TO_POINTER(width));
    g_free(orig_width);
  }
  if (orig_height) {
    sscanf(orig_height, "%i", &height);
    g_object_set_data(G_OBJECT(job->thumbnail), EOM_THUMBNAIL_ORIGINAL_HEIGHT,
...
static void eom_job_load_dispose(GObject *object) {
  EomJobLoad *job;
...
static void eom_job_load_class_init(EomJobLoadClass *class) {
  GObjectClass *oclass;
...
EomJob *eom_job_load_new(EomImage *image, EomImageData data) {
  EomJobLoad *job;
...
static void eom_job_load_run(EomJob *job) {
  ...
  eom_image_load(EOM_IMAGE(EOM_JOB_LOAD(job)->image), EOM_JOB_LOAD(job)->data,
                 job, &job->error);
...
static void eom_job_model_class_init(EomJobModelClass *class) {
...
static void filter_files(GSList *files, GList **file_list, GList **error_list) {
  GSList *it;
  GFileInfo *file_info;
    ...
    switch (type) {
      ...
      case G_FILE_TYPE_DIRECTORY:
        *file_list = g_list_prepend(*file_list, g_object_ref(file));
        break;
      default:
        *error_list = g_list_prepend(*error_list, g_file_get_uri(file));
        break;
    }
  }
...
static void eom_job_transform_dispose(GObject *object) {
  EomJobTransform *job;
...
static void eom_job_transform_class_init(EomJobTransformClass *class) {
  GObjectClass *oclass;
...
EomJob *eom_job_transform_new(GList *images, EomTransform *trans) {
  EomJobTransform *job;
...
static gboolean eom_job_transform_image_modified(gpointer data) {
...
void eom_job_transform_run(EomJob *ejob) {
  EomJobTransform *job;
  GList *it;
...
static void eom_job_save_dispose(GObject *object) {
  EomJobSave *job;
...
static void eom_job_save_class_init(EomJobSaveClass *class) {
...
static void save_progress_handler(EomImage *image, gfloat progress,
                                  gpointer data) {
...
static void eom_job_save_run(EomJob *ejob) {
  EomJobSave *job;
  GList *it;
  ...
  for (it = job->images; it != NULL; it = it->next, job->current_pos++) {
    ...
    if (!eom_image_has_data(image, EOM_IMAGE_DATA_ALL)) {
      ...
      if (data2load != 0) {
        eom_image_load(image, data2load, NULL, &ejob->error);
      }
    }

    handler_id = g_signal_connect(image, "save-progress",
                                  G_CALLBACK(save_progress_handler), job);

    save_info = eom_image_save_info_new_from_image(image);

    success = eom_image_save_by_info(image, save_info, &ejob->error);

    if (save_info) g_object_unref(save_info);

    if (handler_id != 0) g_signal_handler_disconnect(image, handler_id);
...
static void eom_job_save_as_class_init(EomJobSaveAsClass *class) {
...
EomJob *eom_job_save_as_new(GList *images, EomURIConverter *converter,
                            GFile *file) {
  EomJobSaveAs *job;
...
static void eom_job_save_as_run(EomJob *ejob) {
  EomJobSave *job;
  ...
  for (it = job->images; it != NULL; it = it->next, job->current_pos++) {
    ...
    if (n_images == 1) {
      ...
      dest_info = eom_image_save_info_new_from_file(saveas_job->file, format);
      ...
      result = eom_uri_converter_do(saveas_job->converter, image, &dest_file,
                                    &format, NULL);

      g_assert(result);

      dest_info = eom_image_save_info_new_from_file(dest_file, format);
    }

    success =
        eom_image_save_as_by_info(image, src_info, dest_info, &ejob->error);

    if (src_info) g_object_unref(src_info);

    if (dest_info) g_object_unref(dest_info);
...
static void eom_job_copy_dispose(GObject *object) {
...
static void eom_job_copy_class_init(EomJobCopyClass *class) {
...
EomJob *eom_job_copy_new(GList *images, const gchar *dest) {
  EomJobCopy *job;
...
static void eom_job_copy_progress_callback(goffset current_num_bytes,
  ...
  job_progress =
      ((current_num_bytes / (gfloat)total_num_bytes) + job->current_pos) /
      n_images;

  eom_job_set_progress(EOM_JOB(job), job_progress);
}

void eom_job_copy_run(EomJob *ejob) {
  EomJobCopy *job;
  GList *it;
  GFile *src, *dest;
  ...
  for (it = job->images; it != NULL; it = g_list_next(it), job->current_pos++) {
    ...
    g_file_copy(src, dest, G_FILE_COPY_OVERWRITE, NULL,
                eom_job_copy_progress_callback, job, &ejob->error);
    g_free(filename);
