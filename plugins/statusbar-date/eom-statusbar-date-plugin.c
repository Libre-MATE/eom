...
static void eom_window_activatable_iface_init(
    EomWindowActivatableInterface *iface);

G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    EomStatusbarDatePlugin, eom_statusbar_date_plugin, PEAS_TYPE_EXTENSION_BASE,
    0,
...
static void selection_changed_cb(EomThumbView *view,
                                 EomStatusbarDatePlugin *plugin) {
  statusbar_set_date(GTK_STATUSBAR(plugin->statusbar_date), view);
}

static void eom_statusbar_date_plugin_set_property(GObject *object,
                                                   guint prop_id,
                                                   const GValue *value,
                                                   GParamSpec *pspec) {
...
static void eom_statusbar_date_plugin_get_property(GObject *object,
...
static void eom_statusbar_date_plugin_dispose(GObject *object) {
  EomStatusbarDatePlugin *plugin = EOM_STATUSBAR_DATE_PLUGIN(object);
...
static void eom_statusbar_date_plugin_activate(
    EomWindowActivatable *activatable) {
  ...
  plugin->signal_id =
      g_signal_connect_after(G_OBJECT(thumbview), "selection_changed",
...
static void eom_statusbar_date_plugin_deactivate(
    EomWindowActivatable *activatable) {
  EomStatusbarDatePlugin *plugin = EOM_STATUSBAR_DATE_PLUGIN(activatable);
  EomWindow *window = plugin->window;
  GtkWidget *statusbar = eom_window_get_statusbar(window);
...
static void eom_statusbar_date_plugin_class_init(
    EomStatusbarDatePluginClass *klass) {
...
static void eom_statusbar_date_plugin_class_finalize(
    EomStatusbarDatePluginClass *klass) {
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, EOM_TYPE_WINDOW_ACTIVATABLE, EOM_TYPE_STATUSBAR_DATE_PLUGIN);
}
