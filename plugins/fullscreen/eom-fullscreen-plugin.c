...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    EomFullscreenPlugin, eom_fullscreen_plugin, PEAS_TYPE_EXTENSION_BASE, 0,
    G_IMPLEMENT_INTERFACE_DYNAMIC(EOM_TYPE_WINDOW_ACTIVATABLE,
                                  eom_window_activatable_iface_init))

enum { PROP_0, PROP_WINDOW };

static gboolean on_button_press(GtkWidget *button, GdkEventButton *event,
                                EomWindow *window) {
...
static void eom_fullscreen_plugin_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec) {
...
static void eom_fullscreen_plugin_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *pspec) {
  EomFullscreenPlugin *plugin = EOM_FULLSCREEN_PLUGIN(object);
...
static void eom_fullscreen_plugin_dispose(GObject *object) {
  EomFullscreenPlugin *plugin = EOM_FULLSCREEN_PLUGIN(object);
...
static void eom_fullscreen_plugin_activate(EomWindowActivatable *activatable) {
  EomFullscreenPlugin *plugin = EOM_FULLSCREEN_PLUGIN(activatable);
  GtkWidget *view = eom_window_get_view(plugin->window);

  eom_debug(DEBUG_PLUGINS);

  plugin->signal_id = g_signal_connect(
      view, "button-press-event", G_CALLBACK(on_button_press), plugin->window);
}

static void eom_fullscreen_plugin_deactivate(
...
static void eom_fullscreen_plugin_class_finalize(
    EomFullscreenPluginClass *klass) {
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, EOM_TYPE_WINDOW_ACTIVATABLE, EOM_TYPE_FULLSCREEN_PLUGIN);
}
