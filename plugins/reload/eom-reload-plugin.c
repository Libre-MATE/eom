...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    EomReloadPlugin, eom_reload_plugin, PEAS_TYPE_EXTENSION_BASE, 0,
...
static const GtkActionEntry action_entries[] = {
    {"EomPluginRunReload", "view-refresh", N_("Reload Image"), "R",
     N_("Reload current image"), G_CALLBACK(reload_cb)}};

static void eom_reload_plugin_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
...
static void eom_reload_plugin_get_property(GObject *object, guint prop_id,
...
static void eom_reload_plugin_dispose(GObject *object) {
  EomReloadPlugin *plugin = EOM_RELOAD_PLUGIN(object);
...
static void eom_reload_plugin_activate(EomWindowActivatable *activatable) {
  ...
  plugin->ui_id =
      gtk_ui_manager_add_ui_from_string(manager, ui_definition, -1, NULL);
  g_warn_if_fail(plugin->ui_id != 0);
}

static void eom_reload_plugin_deactivate(EomWindowActivatable *activatable) {
  EomReloadPlugin *plugin = EOM_RELOAD_PLUGIN(activatable);
  GtkUIManager *manager;
...
static void eom_reload_plugin_class_finalize(EomReloadPluginClass *klass) {
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, EOM_TYPE_WINDOW_ACTIVATABLE, EOM_TYPE_RELOAD_PLUGIN);
}
