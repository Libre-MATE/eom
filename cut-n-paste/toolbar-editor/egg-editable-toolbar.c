...
static void drag_begin_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  flags = egg_toolbars_model_get_name_flags(etoolbar->priv->model, action_name);
  if (!(flags & EGG_TB_MODEL_NAME_INFINITE)) {
    flags &= ~EGG_TB_MODEL_NAME_USED;
    egg_toolbars_model_set_name_flags(etoolbar->priv->model, action_name,
                                      flags);
  }
}
...
static void drag_end_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  if (gtk_widget_get_parent(widget) != NULL) {
    ...
    flags =
        egg_toolbars_model_get_name_flags(etoolbar->priv->model, action_name);
    if (!(flags & EGG_TB_MODEL_NAME_INFINITE)) {
      flags |= EGG_TB_MODEL_NAME_USED;
      egg_toolbars_model_set_name_flags(etoolbar->priv->model, action_name,
                                        flags);
...
static void drag_data_get_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  data = egg_toolbars_model_get_data(
      model, gtk_selection_data_get_target(selection_data), name);
  if (data != NULL) {
...
static void move_item_cb(GtkAction *action, EggEditableToolbar *etoolbar) {
  ...
  gtk_drag_begin_with_coordinates(toolitem, list, GDK_ACTION_MOVE, 1,
...
static void toolbar_drag_data_received_cb(GtkToolbar *toolbar,
  ...
  /* Find out where the drop is occurring, and the name of what is being
   * dropped. */
...
static void toolbar_visibility_refresh(EggEditableToolbar *etoolbar) {
  ...
  for (i = 0; i < n_toolbars; i++) {
    ...
    gtk_toggle_action_set_active(
        toggle_action, gtk_widget_get_visible(get_dock_nth(etoolbar, i)));
